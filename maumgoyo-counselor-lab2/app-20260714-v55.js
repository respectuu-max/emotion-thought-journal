...
const VIEWS = [
  ...
  ["prediction", "8. 예기불안 검증", "예상한 심각도와 실제 결과, 파국예상 재검토"],
...
function ingestCsv(text, fileName) {
  const incomingRows = parseCsv(text);
  ...
  state.importMessages = [mergeNotice, dedupeNotice, ...validation.warnings].filter(Boolean);
...
// 평균 충동·평균 문제행동 수준을 집계해, 가장 위험도가 높은 시간대/요일을 짚어줍니다.
...
function clientAliasesForMerge(rows) {
  return unique(rows.map((row) => String(row.client_alias || "").trim()));
}
...
function buildAccumulatedCsvFileName() {
  const clientAlias = state.chains[0]?.client || state.records[0]?.client || "내담자";
...
function exportAccumulatedCsv() {
  ...
  downloadText(buildAccumulatedCsvFileName(), serializeRowsToCsv(state.rows), "text/csv;charset=utf-8");
}
...
function validateRows(rows) {
  ...
  if (blankClientAliasCount) warnings.push(`client_alias가 비어 있는 행이 ${blankClientAliasCount}개 있습니다. 이 파일 하나는 분석할 수 있지만, 안전한 내담자 확인이 불가능하므로 다른 주차 CSV와 자동 누적 병합하지 않습니다.`);
...
function renderMetrics() {
  ...
  const metrics = [
    ...
    ["충동 감내력", tolerance,
      `<b>충동은 높았지만 중독 행동으로 넘어가지 않은</b> 기록입니다. 충동을 없애거나 억압했다는 뜻이 아니라, 충동이 있는 상태에서도 <b>감내하고 버티며 가치와 안전을 선택한 힘</b>을 보여줍니다.`],
    ["실천행동 조정 필요", missed,
    ...
    ["정서 주의", emotionalWarnings,
      `${metricBadge(emotionalEvidence.label, emotionalEvidence.className)}<b>감정 강도 ${t.emotion}/10 이상</b>, 부정정서 표현, 실천 저수행이 잡힌 기록입니다. ${escapeHtml(emotionalEvidence.note)} 행동화가 없더라도 마음 안에서는 이미 부담이 커졌을 수 있어 <b>정서 조절과 몸반응 안정화</b>를 확인합니다.`],
...
function renderIntro(viewId) {
  const intro = {
    data: {
      purpose: "자료를 해석하기 전에 기록량, 기간, CSV 구조를 확인해 이번 회기에서 어느 정도까지 말할 수 있는지 정합니다.",
      metrics: ["자료량"],
      guide: ["자료 대표성 확인", "record_type 구성 확인", "민감정보와 보관 원칙 확인", "오늘 다룰 기간 합의"],
      questions: ["이 자료가 이번 주 생활을 어느 정도 대표하나요?", "기록이 적은 날은 기록이 없었던 것인가요, 공유되지 않은 것인가요?", "오늘은 어떤 기간의 변화를 중심으로 보고 싶나요?"],
      feedback: "자료량이 적으면 결론보다 가설로 제시하고, 자료량이 충분하면 반복 패턴을 함께 찾습니다.",
    },
    feedback: {
      purpose: "측정치를 평가표가 아니라 거울처럼 사용해 내담자가 자신의 변화, 위험, 가능성을 직접 발견하도록 돕습니다.",
      metrics: ["자료량", "위험 연쇄", "충동 감내력", "변화 포착 단서"],
      guide: ["그래프 먼저 보기", "좋아진 점 먼저 찾기", "주의 신호 확인", "다음 주 측정 초점 합의"],
      questions: ["그래프에서 가장 먼저 보이는 변화는 무엇인가요?", "문제가 줄어든 지점은 어디이며 무엇이 도움이 되었나요?", "주의해야 할 신호는 어느 시점부터 보이나요?"],
      feedback: "위험 수치만 강조하지 않고, 내담자가 이미 해낸 작은 변화와 충동 감내 경험을 함께 피드백합니다.",
    },
    readiness: {
      purpose: "자료를 바로 제시하기보다 내담자가 들을 준비가 되어 있는지 살피고, 변화언어를 키우는 방향으로 피드백합니다.",
      metrics: ["변화 포착 단서", "실천행동 조정 필요"],
      guide: ["변화언어 확인", "유지언어 반영", "양가감정 요약", "피드백 허락 구하기"],
      questions: ["이 변화가 계속된다면 어떤 점이 달라질까요?", "계속 미루게 만드는 현실적 이유는 무엇인가요?", "제가 자료를 보고 느낀 점을 함께 확인해도 괜찮을까요?"],
      feedback: "반박이나 설득보다 자율성, 선택권, 이미 나타난 변화의 의미를 먼저 반영합니다.",
    },
    safety: {
      purpose: "정서·인지 신호가 충동과 행동화로 커지기 전에 멈춤, 도움요청, STOP/TIPP 호흡훈련을 연결합니다.",
      metrics: ["위험 연쇄", "정서 주의", "인지 주의"],
      guide: ["정서·인지 신호 확인", "행동화 임박 지점 찾기", "STOP/TIPP 후보 선택", "조기 도움요청 계획"],
      questions: ["위기 직전 가장 먼저 알아차릴 수 있었던 신호는 무엇인가요?", "그때 30초 호흡에 집중하지 못하게 한 방해요인은 무엇이었나요?", "다음에는 누구에게, 어느 시점에 도움을 요청할까요?"],
      feedback: "위기대처는 실패를 추궁하는 시간이 아니라, 다음 위기에서 더 일찍 멈추는 지점을 찾는 훈련입니다.",
    },
    chain: {
      purpose: "중독 행동을 의지 부족으로 보지 않고, 상황-생각-감정/몸-충동-행동/대처의 반복 경로로 이해합니다.",
      metrics: ["연쇄 고리", "정서 주의", "인지 주의", "위험 연쇄"],
      guide: ["트리거 확인", "생각-감정-몸반응 연결", "충동과 행동화 지점 찾기", "개입 가능한 한 지점 선택"],
      questions: ["이 연쇄에서 가장 먼저 바꿀 수 있는 작은 지점은 어디인가요?", "충동이 올라가기 전 마음속 문장은 무엇이었나요?", "몸반응을 더 일찍 알아차린다면 무엇이 달라질까요?"],
      feedback: "연쇄를 보이면 내담자는 문제를 자기비난이 아니라 수정 가능한 패턴으로 이해할 수 있습니다.",
    },
    success: {
      purpose: "완전한 무증상만 성공으로 보지 않고, 충동 속에서 버틴 경험과 행동 강도가 낮아진 변화를 회복 증거로 강화합니다.",
      metrics: ["충동 감내력", "변화 포착 단서", "위험 연쇄"],
      guide: ["충동 감내 경험 찾기", "행동 강도 감소 확인", "도움된 조건 추출", "다음 주 재현 계획"],
      questions: ["충동이 있었는데도 넘어가지 않은 순간에 무엇이 달랐나요?", "문제가 10에서 7로 줄었다면 그 3만큼의 변화는 무엇이 만들었나요?", "다음 주에도 같은 조건을 만들려면 무엇이 필요할까요?"],
      feedback: "내담자가 자기효능감을 느끼도록, 결과보다 감내한 과정과 강도 감소의 의미를 구체적으로 반영합니다.",
    },
    practice: {
      purpose: "실천행동 조정 필요를 실패가 아니라 가치 있는 삶을 향한 과제 재설계 신호로 사용합니다.",
      metrics: ["실천행동 조정 필요", "변화 포착 단서", "충동 감내력"],
      guide: ["저수행 이유를 비난 없이 확인", "작게 줄일 행동 선택", "가치와 연결", "새 경험이 생각을 바꾸는 방식 확인"],
      questions: ["이 실천이 너무 컸다면 2분짜리 행동으로 줄이면 무엇이 될까요?", "이 행동을 하면 어떤 삶의 가치 쪽으로 1% 움직이나요?", "작게라도 해본 뒤 나에 대한 생각이 조금 달라진 지점은 무엇인가요?"],
      feedback: "작은 실천은 단순 과제가 아니라 새로운 경험을 만들고, 그 경험이 자기개념과 자동사고를 수정하는 회복 훈련입니다.",
    },
    prediction: {
      purpose: "파국예상과 실제 결과를 비교해 불안, 재발예상, 자기불신이 얼마나 정확했는지 자료로 검토합니다.",
      metrics: ["예기불안 검증", "인지 주의"],
      guide: ["예상 확인", "실제 결과 비교", "차이 해석", "다음 예측에 적용"],
      questions: ["예상은 실제보다 얼마나 컸나요?", "일어나지 않은 걱정이 있었다면 그것은 무엇을 말해주나요?", "다음에 같은 걱정이 올 때 참고할 근거 문장은 무엇인가요?"],
      feedback: "예상과 실제의 차이를 내담자의 착각으로 몰지 않고, 불안이 만들어낸 예측을 함께 검증하는 실험으로 다룹니다.",
    },
    summary: {
      purpose: "회기 말에는 위험, 변화, 실천 조정, 다음 주 측정 초점을 한 장으로 정리해 회복 방향을 유지합니다.",
      metrics: ["자료량", "위험 연쇄", "충동 감내력", "실천행동 조정 필요", "변화 포착 단서"],
      guide: ["오늘의 위험 1개", "오늘의 변화 1개", "다음 주 실천 1개", "다음 회기 확인 지표"],
      questions: ["오늘 자료를 보고 자신에 대해 새롭게 알게 된 점은 무엇인가요?", "다음 주에 지키고 싶은 가장 작은 행동은 무엇인가요?", "다음 회기에서 어떤 수치가 달라졌는지 보고 싶나요?"],
      feedback: "요약은 평가표가 아니라 내담자가 다음 주에 붙잡을 회복 언어와 행동 약속을 만드는 과정입니다.",
    },
  }[viewId];
...
function metricRoutingCard() {
  const routes = [
    ...
    ["연쇄 고리", "5. 연쇄 이해", "반복되는 마음-행동 경로를 상담 주제로 바꿉니다."],
    ["위험 연쇄", "4. 위기대처 · 5. 연쇄 이해", "재발예방, STOP/TIPP, 도움요청 계획으로 연결합니다."],
    ["충동 감내력", "6. 성공·강도감소", "충동 속에서도 버틴 힘을 회복 증거로 강화합니다."],
    ["실천행동 조정 필요", "7. 가치기반 실천", "실패가 아니라 더 작은 가치 행동으로 재설계합니다."],
    ["변화 포착 단서", "2. 측정 피드백 · 6. 성공 · 7. 실천", "좋아진 점, 숨어 있는 위험, 동기저하를 피드백합니다."],
    ["정서 주의", "4. 위기대처 · 5. 연쇄 이해", "감정과 몸반응이 충동으로 커지는 지점을 다룹니다."],
    ["인지 주의", "4. 위기대처 · 8. 예기불안 검증", "허용 생각, 합리화, 파국예상을 CBT/ACT 질문으로 검토합니다."],
    ["예기불안 검증", "8. 예기불안 검증", "예상과 실제 결과를 비교해 불안 예측을 재평가합니다."],
  ];
...
function postActionRecoveryCard() {
  return card(
    "행동화 이후 회복 복귀",
    `<p>행동화가 있었다면 회기는 처벌이나 추궁이 아니라 <strong>다시 회복 경로로 돌아오는 능력</strong>을 키우는 시간이 되어야 합니다.</p><p>행동화 여부만 보지 않고, 버틴 시간, 손상을 줄인 행동, 도움요청 가능성, 다음 복귀 지점을 함께 찾습니다.</p>`,
    "warn",
    [
      "행동화 전 어느 지점까지는 버틸 수 있었나요?",
...
function recoveryIdentityCard() {
  return card(
    "회복 정체성 질문",
    `<p>장기 회복은 문제행동을 줄이는 것을 넘어 <strong>나는 어떤 사람으로 살아가고 있는가</strong>가 바뀌는 과정입니다. 작은 실천과 충동 감내 경험을 새로운 자기이해로 연결합니다.</p>`,
    "ok",
    [
...
function renderMeasurementFeedbackView() {
  ...
  const success = chains(isDistressTolerance).slice(0, 3).map((chain) => chainCard(chain, "충동 감내력 피드백", "ok"));
  ...
  const weeklyVisual = visualFeedback(
    "주별 변화 피드백",
    "여러 주차 CSV를 누적했을 때, 주별 평균 충동·문제행동·실천 기록·성취감·삶의 확장감을 비교합니다.",
  ...
  const visual = visualFeedback(
    ...
    renderBarSummary([
      ...
      ["충동 감내", chains(isDistressTolerance).length * 20, "ok"],
  ...
  return visual + weeklyVisual + scatterVisual + calendarVisual + dualAxisVisual + expansionScatterVisual + workbench([
...
function renderSuccessView() {
  ...
  return visual + workbench([
    ...
    ["충동 감내력 기록", success.map((chain) => chainCard(chain, "충동을 감내한 기록", "ok"))],
...
// 충동/문제행동/실천완료율/확장감 평균 — 목표 대비 진행률과 지난 회기 비교가 공유하는 핵심 집계입니다.
...
function renderSessionFeedbackSummaryView() {
  ...
  return visual + workbench([
    ...
    ["오늘의 변화", success.map((chain) => chainCard(chain, isPartialSuccess(chain) ? "강도감소 성공" : "충동 감내력", "ok")).concat(changeTalk.map((item) => recordCard(item.record, item.label, "ok")))],
...
function renderWeeklyStatsTable() {
  ...
  return `<div class="weekly-table-wrap"><table class="weekly-table">
    <thead><tr><th>주차</th><th>충동</th><th>문제행동</th><th>실천</th><th>성취감</th><th>확장감</th><th>위험기록</th></tr></thead>
    <tbody>${rows}</tbody>
  </table></div>`;
}
...
// "요약 저장" JSON 파일을 다시 불러와 이번 주와 비교합니다. 오늘 이후 저장되는 요약에는
...
const SNAPSHOT_METRICS = [
  ["avgUrge", "충동 평균", "", false],
  ["avgAction", "문제행동 평균", "", false],
  ["practiceCompletionRate", "실천 완료율", "%", true],
  ["avgExpansion", "삶의 확장감 평균", "", true],
...
function renderSessionComparison() {
  ...
  const legacyNote = prev.isLegacy
    ? `<p class="muted">이전 요약이 이번 비교 기능 도입 이전 형식이라, 확장감·변화언어 비율은 비교할 수 없습니다.</p>`
    : "";
...
function exportSummary() {
  ...
  downloadText(buildSummaryFileName(), JSON.stringify(payload, null, 2));
}
...
function buildSummaryText() {
  ...
  lines.push(
    ...
    `충동 감내력: ${chains(isDistressTolerance).length}개`,
...
function buildSummaryFileName() {
  const clientAlias = state.chains[0]?.client || state.records[0]?.client || "내담자";
  return `${sanitizeForFilename(clientAlias)}_${computeDataDateRangeLabel()}_요약.json`;
}
