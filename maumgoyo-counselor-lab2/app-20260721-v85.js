const APP_VERSION = "maeumgoyo-counselor-lab-20260721v85";
...
function bindEvents() {
  ...
  els.previousSummaryInput.addEventListener("change", (event) => {
    ...
    reader.onload = () => {
      ...
      showImportMessages([`지난 회기 비교용 요약(${snapshot.fileName || file.name})을 불러왔습니다.${snapshot.isLegacy ? " (예전 형식 — 일부 지표만 비교 가능)" : ""}`]);
      render();
    };
