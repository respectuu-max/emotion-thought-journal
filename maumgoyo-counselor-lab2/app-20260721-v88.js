const APP_VERSION = "maeumgoyo-counselor-lab-20260721v88";
const AI_EVIDENCE_SCHEMA = "maeumgoyo_ai_evidence_v1";
...
const state = {
  ...
  lastSaveFileHandle: null,
  lastSaveSignature: "",
  lastSaveRequestedAt: 0,
  ...
  // 개인별 임계값 보정: 이 내담자 자신의 자기보고 분포에서 계산한 제안값과, 상담자가 실제로
  // 확인·적용한 값을 함께 기록합니다. action(문제행동수준)은 안전망이라 개인화 대상에서 제외합니다.
...
const els = {
  ...
  sessionAttendanceInput: document.getElementById("sessionAttendanceInput"),
  assignedTasksInput: document.getElementById("assignedTasksInput"),
  counselorVerifiedInput: document.getElementById("counselorVerifiedInput"),
  verificationLimitsInput: document.getElementById("verificationLimitsInput"),
  ...
  aiEvidenceExportBtn: document.getElementById("aiEvidenceExportBtn"),
  evidencePreflightBtn: document.getElementById("evidencePreflightBtn"),
  evidencePreflightResult: document.getElementById("evidencePreflightResult"),
  promptProfileSelect: document.getElementById("promptProfileSelect"),
  promptProfileDescription: document.getElementById("promptProfileDescription"),
  copySelectedPromptBtn: document.getElementById("copySelectedPromptBtn"),
  saveSelectedPromptBtn: document.getElementById("saveSelectedPromptBtn"),
  promptReadinessBtn: document.getElementById("promptReadinessBtn"),
  promptReadinessResult: document.getElementById("promptReadinessResult"),
  ...
  clientStorageNameInput: document.getElementById("clientStorageNameInput"),  storageFolderStatus: document.getElementById("storageFolderStatus"),
  ...
  interventionBranchNote: document.getElementById("interventionBranchNote"),
  relationalDisclosureStatus: document.getElementById("relationalDisclosureStatus"),
  relationalRepairStatus: document.getElementById("relationalRepairStatus"),
  relationalImpactNote: document.getElementById("relationalImpactNote"),
  ...
  previousSummaryInput: document.getElementById("previousSummaryInput"),  sessionComparisonResult: document.getElementById("sessionComparisonResult"),
  ...
  safetyScreeningStatus: document.getElementById("safetyScreeningStatus"),
  safetyScreeningNote: document.getElementById("safetyScreeningNote"),
  safetyScreeningBanner: document.getElementById("safetyScreeningBanner"),
  thresholdCalibrationSuggestion: document.getElementById("thresholdCalibrationSuggestion"),
  applyPersonalizedThresholdsBtn: document.getElementById("applyPersonalizedThresholdsBtn"),
  thresholdProfileStatus: document.getElementById("thresholdProfileStatus"),
};

const VIEWS = [
  ["data", "1. 자료와 치료 프레임 확인", "CSV 구성, 자료 범위, 감정 감내력 창"],
  ["feedback", "2. 오늘 마음에 걸린 경험 지도", "경험한 문제, 정서/각성 흐름, 변화 단서"],
  ["chain", "3. 감정 중심 자동 연쇄 재구성", "경험한 문제-감정-생각-몸-충동-문제행동수준"],
...
const SESSION_TASKS = {
  ...
  chain: [
    ...
    "경험한 문제 → 상황 → 감정 → 생각 → 몸반응 → 충동 → 문제행동수준 → 대처 → 가치 방향으로 따라갑니다.",
...
function bindEvents() {
  ...
  if (els.applyPersonalizedThresholdsBtn) {
    els.applyPersonalizedThresholdsBtn.addEventListener("click", () => {
  ...
  if (els.aiEvidenceExportBtn) els.aiEvidenceExportBtn.addEventListener("click", exportAiEvidencePackage);
  if (els.evidencePreflightBtn) els.evidencePreflightBtn.addEventListener("click", runEvidencePreflight);
  if (els.promptProfileSelect) els.promptProfileSelect.addEventListener("change", renderPromptProfileDescription);
  if (els.copySelectedPromptBtn) els.copySelectedPromptBtn.addEventListener("click", copySelectedPromptProfile);
  if (els.saveSelectedPromptBtn) els.saveSelectedPromptBtn.addEventListener("click", saveSelectedPromptProfile);
  if (els.promptReadinessBtn) els.promptReadinessBtn.addEventListener("click", runSelectedPromptReadiness);
  renderPromptProfileDescription();
  [
    els.sessionDateInput,
    els.sessionAttendanceInput,
    els.safetyScreeningStatus,
    els.safetyScreeningNote,
    els.assignedTasksInput,
    els.counselorVerifiedInput,
    els.verificationLimitsInput,
    els.notes,
    els.projectNameInput,
    els.clientStorageNameInput,
    els.acceptedAiSuggestions,
    els.deferredAiSuggestions,
    els.nextSessionQuestions,
    els.nextAiContext,
    els.interventionBranchNote,
    els.relationalDisclosureStatus,
    els.relationalRepairStatus,
    els.relationalImpactNote,
  ].forEach((input) => {
    if (!input) return;
    input.addEventListener(input.tagName === "SELECT" ? "change" : "input", invalidatePreflightResults);
  });

  // 안전 선별 상태는 자동 초점 판정과 무관하게 즉시 화면 최상단 배너에 반영되어야 합니다.
  if (els.safetyScreeningStatus) {
    els.safetyScreeningStatus.addEventListener("change", () => {
      renderSafetyScreeningBanner();
    });
  }
  // 관계적 영향이 "발각됨"으로 바뀌는 시점은 배신외상·재정위기 문헌에서 급성 자살위험이
  // 몰리는 시기로 보고되는 지점이라, 안전 선별을 다시 확인하도록 상기시킵니다.
  if (els.relationalDisclosureStatus) {
    els.relationalDisclosureStatus.addEventListener("change", () => {
      if (els.relationalDisclosureStatus.value === "discovered") {
        showActionToast("발각됨으로 표시했습니다 — 자살·자해 위험 선별을 이번 회기 기준으로 다시 확인해 주세요.", "warning");
        els.safetyScreeningStatus?.focus();
      }
    });
  }
  ...
  if (els.clientStorageNameInput) els.clientStorageNameInput.addEventListener("input", () => {
    state.clientStorageName = els.clientStorageNameInput.value.trim();
    renderMeta();
  ...
  els.previousSummaryInput.addEventListener("change", (event) => {
    ...
    reader.onload = () => {
      ...
      try {
        ...
        showImportMessages(["지난 회기 비교용 요약 파일의 형식이 올바르지 않습니다. JSON 파일인지 확인하세요."]);
        return;
      }
      ...
      const messages = [`지난 회기 비교용 요약(${snapshot.fileName || file.name})을 불러왔습니다.${snapshot.isLegacy ? " (예전 형식 — 일부 지표만 비교 가능)" : ""}`];
      if (snapshot.thresholdProfile) {
        const before = { ...state.thresholds };
        const p = snapshot.thresholdProfile;
        state.thresholds.urge = p.values.urge;
        state.thresholds.emotion = p.values.emotion;
        state.thresholds.cognitiveUrge = p.values.cognitiveUrge;
        els.urgeThreshold.value = state.thresholds.urge;
        els.emotionThreshold.value = state.thresholds.emotion;
        els.cognitiveUrgeThreshold.value = state.thresholds.cognitiveUrge;
        state.thresholdProfile = p;
        const stale = isThresholdProfileStale(p);
        messages.push(
          `지난 회기 개인화 임계값을 불러왔습니다: 충동 ${before.urge}→${state.thresholds.urge}, 정서 ${before.emotion}→${state.thresholds.emotion}, 인지주의 ${before.cognitiveUrge}→${state.thresholds.cognitiveUrge}${stale ? " (4주 이상 지나 재보정을 권장합니다)" : ""}`,
        );
        rebuildChains();
      }
    ...
    reader.onerror = () => showImportMessages(["지난 회기 비교용 요약 파일을 읽지 못했습니다."]);
...
// gratitude_others/gratitude_self는 앱 버전에 따라 세 가지 형태로 올 수 있습니다(감사필드 작업명세서 §4).
// v100 이전: 필드 없음 → 빈 배열 / v100: 문자열 하나 → 그 문자열 하나를 담은 배열 / v101부터: 문자열 배열 → 그대로.
...
// 감사 기록은 daily_checkin 전체를 훑어 건수·기록일·대표 원문을 뽑아내는 별도 집계입니다.
// SNAPSHOT_METRICS(개선/악화 자동판정 대상)에는 절대 포함하지 않습니다 — 선택 입력 항목이라
// 건수만으로 "개선/악화"를 매기면 안 적은 주가 자동으로 "악화"로 표시되는 왜곡이 생깁니다.
function computeGratitudeMeasures() {
  const checkins = state.dailyCheckins || [];
  const othersEntries = [];
  const selfEntries = [];
  checkins.forEach((entry) => {
    entry.gratitudeOthers.forEach((text) => othersEntries.push({ date: entry.date, text, evidenceRef: `SR-${sanitizeEvidenceId(entry.id)}` }));
    entry.gratitudeSelf.forEach((text) => selfEntries.push({ date: entry.date, text, evidenceRef: `SR-${sanitizeEvidenceId(entry.id)}` }));
  });
  const daysWithOthers = new Set(othersEntries.map((e) => e.date)).size;
  const daysWithSelf = new Set(selfEntries.map((e) => e.date)).size;
  const daysWithAny = new Set([...othersEntries, ...selfEntries].map((e) => e.date)).size;
  return {
    checkinDays: checkins.length,
    othersCount: othersEntries.length,
    selfCount: selfEntries.length,
    daysWithGratitudeOthers: daysWithOthers,
    daysWithGratitudeSelf: daysWithSelf,
    daysWithAnyGratitude: daysWithAny,
    othersEntries,
    selfEntries,
  };
}

// 대표 감사 내용: 최신 항목부터 최대 limit개. "좋은/의미 있는" 내용을 앱이 골라내는 것이 아니라
// 단순히 최근 순으로 뽑는 것이며, 이 선정 자체가 해석이 되지 않도록 최신순 외 다른 기준은 두지 않습니다.
function pickRepresentativeGratitude(entries, limit = 5) {
...
// --- 시간대·요일 위험 패턴 ---
// time_slot("충동 발생"은 관찰 방식 표시일 뿐 실제 시간대가 아니므로 제외)과 관찰 날짜의 요일별로
...
function computeTimeRiskPatterns() {
  ...
  state.chains.forEach((chain) => {
    ...
    if (payload.time_slot && payload.time_slot !== "충동 발생") {
      if (!bySlot.has(payload.time_slot)) bySlot.set(payload.time_slot, { urgeSum: 0, urgeCount: 0, actionSum: 0, actionCount: 0, count: 0 });
      const entry = bySlot.get(payload.time_slot);
      entry.count += 1;
...
function buildAccumulatedCsvFileName() {
  return `누적_원본자료_${sanitizeForFilename(getStorageLabel())}_${computeDataDateRangeLabel()}.csv`;
}
  ...
  parsed.forEach(({ row, payload, index }) => {
    ...
    if (row.record_type === "observation") {
      ...
      pushRecord(
        ...
        joinText([payload.situation, triggerText ? `촉발단서: ${triggerText}` : "", behaviorAreas ? `경험한 문제: ${behaviorAreas}` : "", avoidanceText ? `회피 신호: ${avoidanceText}` : ""]),
      );
      ...
      pushRecord(
        records, base, chainId, "action", "문제행동수준",
...
function renderMeta() {
  ...
  const privacy = state.shareMode.includes("backup_full")
    ? "기간 제한 없이 내보낸 전체 자료입니다. 앱은 보관 처리된 기록을 제외하고 활성 기록만 분석합니다. 실제 문장이 포함될 수 있으므로 보안 폴더에 저장하세요."
    : "상황, 생각, 감정, 몸반응, 충동, 문제행동수준, 실천행동이 포함된 치료 분석용 자료입니다. 자유서술에 민감정보가 있을 수 있으므로 저장·전송에 주의하세요.";
  ...
  const infoList = [
    ...
    ["AI 전달자료 이름표", state.fileName ? getProjectName() : "-"],
    ["이전 회기연결 메모", state.previousAiContextFileName || "-"],
  ];
  if (state.predictions.length) infoList.push(["예기불안 검증", `${state.predictions.length}개`]);
  els.schemaInfo.innerHTML = infoRows(infoList);
}

function updateCsvStartPrompt() {
  if (!els.dropZone) return;
  const shouldPrompt = !state.fileName && !state.rows.length;
  els.dropZone.classList.toggle("csv-start-prompt", shouldPrompt);
  els.dropZone.setAttribute(
    "aria-label",
    shouldPrompt
      ? "먼저 CSV 파일 불러오기를 눌러 마음고요 관찰과 실천 CSV 자료를 선택하세요."
      : "CSV 파일을 추가로 불러오거나 끌어오세요."
  );
}
...
function showActionMessage(message, type = "success") {
  ...
  els.actionToast.className = `action-toast ${type === "success" ? "" : type}`.trim();
...
function showSaveProblem(message) {
  if (els.saveStatus) {
    ...
    els.saveStatus.classList.add("save-warning");
  }
  showImportMessages([message]);
  showActionMessage(message, "warning");
}
...
function renderDataView() {
  ...
  const structural = [
    card("CSV 구조", `<p>마음고요 관찰과 실천에서 내보낸 현재 형식의 자료를 읽었습니다.</p>`, "focus"),
    card("자료 종류", `<p><strong>${escapeHtml(state.shareModeLabel)}</strong></p><p>치료 회기에서 상황, 생각, 감정, 충동, 실천행동을 함께 검토할 수 있는 자료입니다.</p>`, "ok"),
    card("자료 보호", `<p>불러온 CSV는 이 탭 안에서만 분석됩니다.</p><p>저장 파일에는 실제 기록 문장이 포함될 수 있으므로 내담자별 보안 폴더에 보관하세요.</p>`, "focus"),
...
const SAFETY_SCREENING_LABELS = {
  not_recorded: "기록하지 않음",
  none: "없음",
  passive_ideation: "경미한 사고 있으나 계획·의도 없음",
  high_risk: "위험도 높음(계획·의도·수단 관련 언급)",
};

// 자살·자해 위험 선별은 재발위기 자동 초점 판정(crisisScore 등)과 완전히 별개입니다.
// CSV가 없거나 위험 연쇄가 없어도, 상담자가 이 항목을 "위험도 높음"으로 표시하면
// 화면 최상단에 always-on 배너로 강제 표시합니다. 실제 판단은 전적으로 상담자 몫입니다.
function renderSafetyScreeningBanner() {
  if (!els.safetyScreeningBanner) return;
  const status = els.safetyScreeningStatus?.value || "not_recorded";
  if (status !== "high_risk") {
    els.safetyScreeningBanner.hidden = true;
    els.safetyScreeningBanner.innerHTML = "";
    return;
  }
  els.safetyScreeningBanner.hidden = false;
  els.safetyScreeningBanner.innerHTML = `
    <span class="safety-screening-dot" aria-hidden="true"></span>
    <strong>자살·자해 위험 선별: 위험도 높음으로 표시됨</strong>
    <span class="safety-screening-note">이 배너는 자동 초점 판정과 무관하게 상담자가 직접 표시한 결과입니다. 안전계획 확인·위기상담 연계 등 필요한 조치를 취했는지 확인하세요.</span>
  `;
}
...
const PROFILE_F_PROMPT = `# 프로필 F — 주간 치료참여 및 재범방지 실천기록 생성 프롬프트

## 입력자료

이 프롬프트와 함께 제공되는 JSON은 \`maeumgoyo_ai_evidence_v1\` 형식의 표준 근거자료이다. JSON 이외의 사실을 추가하거나 추정하지 마라.

## 역할과 목적

당신은 상담자가 검토할 「주간 치료참여 및 재범방지 실천기록」 초안을 작성하는 기록 보조 AI이다. 목적은 내담자가 한 주 동안 자기 변화와 재범 방지를 위해 수행한 구체적인 자기관찰, 위험신호 인식, 행동 중단, 대안행동, 상담과제 및 실패 후 수정 노력을 사실 중심으로 구조화하는 것이다.

이 문서는 치료 소견서, 진단서, 재범위험성 평가서, 진정성 판정서 또는 양형 의견서가 아니다.

## 근거 사용 규칙

1. JSON에 존재하는 정보만 사용한다. 없는 내용은 만들지 말고 "제공된 자료만으로 확인하기 어려움"이라고 쓴다.
2. 주요 사실 뒤에 대괄호로 근거 ID를 표시한다. 예: [SR-obs-001], [CO-VERIFY-001], [AM-PRACTICE-001].
3. 출처를 다음과 같이 엄격히 구분한다.
   - SR: 내담자의 자기보고 또는 앱 입력이며 독립적으로 확인된 사실이 아니다.
   - CO: 상담자가 입력한 관찰·확인 사실이다.
   - AM: 앱이 계산하거나 재구성한 보조정보이며 상담자 검토가 필요하다.
   - AI: 보류된 AI 해석으로 사실처럼 사용해서는 안 된다.
   - CH: 상담자가 채택한 해석이지만 원자료 및 상담자 판단과 구분한다.
4. 계획한 행동, 실제 수행한 행동, 부분 수행, 미수행을 서로 구분한다.
5. 기록이 없는 날, 지연 작성, 과제 미실천, 불확실성 및 기록 간 불일치를 숨기지 않는다.
6. "열심히 노력했다", "많이 성찰했다", "치료에 적극적이다" 같은 추상적 평가만 쓰지 말고 날짜·상황·행동·결과를 제시한다.
7. 기록 횟수가 많다는 이유만으로 변화의 진정성이나 치료효과를 인정하지 않는다.
8. 진심으로 반성한다, 완치되었다, 재범 가능성이 낮아졌다, 선처가 필요하다는 결론을 쓰지 않는다.
9. 피해자 또는 제3자의 이름·연락처·주소·직장·학교·구체적 장소 등 식별정보를 삭제하거나 일반화한다.
10. 범죄 또는 성적 행동을 자극적이거나 불필요하게 상세히 재현하지 않는다.
11. 피해자에게 책임을 돌리거나 범행을 축소·정당화하는 표현을 그대로 받아들이지 않는다. 그러한 표현이 자료에 있으면 상담에서 추가 확인할 사항으로 중립적으로 표시한다.
12. 최종 문서의 첫머리와 끝에 "AI 작성 초안 · 상담자 검토 전 · 외부 제출 불가"를 표시한다.

## 반드시 분석할 변화 노력

- 기록의 지속성: 기록일 수, 무기록일, 지연 기록, 특정일 집중 여부
- 자기관찰: 상황·생각·감정·신체반응·충동·행동을 구분한 정도
- 위험상황 조기 인식: 고립, 스트레스, 분노, 수치심, 거절감, 음주, 수면 부족, 디지털 사용 등 자료에서 실제 확인되는 개인별 신호
- 중단 노력: 충동과 행동 사이에 시간을 확보하거나 장소 이동, 기기 차단, 호흡, 기록, 연락, 도움 요청 등을 실행한 사례
- 대안행동: 계획이 아니라 실제 수행한 안전한 행동과 그 결과
- 상담과제 적용: 합의한 과제, 적용 상황, 수행 결과, 장애요인, 피드백 반영
- 책임과 경계: 자신의 선택에 대한 책임 인식, 타인의 동의·경계·안전을 존중하기 위한 구체적 행동 기준
- 실패 후 수정: 미실천·부분실천·회피를 인정하고 다음 행동을 수정한 과정
- 지속가능성: 반복 수행 여부와 다음 주에도 확인할 수 있는 구체적 계획
- 감사 인식과 지지자원 활용: AM-GRATITUDE-001의 타인/자신에 대한 감사 기록 건수와 대표 원문. 이 항목은 선택 입력이므로 건수의 많고 적음 자체를 평가하지 말고, 기록된 원문에 나타난 구체적 지지관계나 자기인식이 있으면 사실로 제시한다.
- 관계적 영향과 회복: relational_impact의 disclosure_status_label/repair_status_label만 사실로 제시한다(예: "공개함, 회복 진행 중"). 상담자 관찰 메모(CO-RELATIONAL-NOTE-001)에 배우자·가족 등 제3자의 이름이나 신원을 짐작할 수 있는 표현이 있으면 절대 그대로 인용하지 말고, 반드시 "제3자와의 관계 회복을 위한 노력이 있었음" 같은 비식별 표현으로 바꾼다. relational_impact가 not_recorded/not_applicable이면 이 항목 자체를 생략한다.

## 출력 형식

# 주간 치료참여 및 재범방지 실천기록

> AI 작성 초안 · 상담자 검토 전 · 외부 제출 불가
...
- 회기 참여 상태:
- 자료 생성일:
- 기록일 수 / 관찰기간 내부 무기록일 수:
- 자료 출처: 내담자 자기보고 / 앱 계산 / 상담자 확인

## 2. 주간 치료참여 개요

5~8문장으로 작성한다. 상담 참석, 자기관찰 기록, 상담과제, 주요 위험상황, 가장 구체적인 재범방지 행동, 미실천 또는 추가 확인 사항을 포함한다. 평가문 대신 근거가 있는 사실을 쓴다.

## 3. 날짜별 구체적인 변화 노력

| 날짜 | 관찰한 상황·위험신호 | 실제로 선택한 행동 | 사용한 중단기술·대안행동 | 결과·학습 | 근거 ID |
|---|---|---|---|---|---|

- 같은 실천을 여러 날 수행했으면 날짜와 반복 횟수를 제시한다.
- "참았다"고만 쓰지 말고, 어떻게 시간을 확보하고 무엇으로 행동을 전환했는지 쓴다.
- 계획만 존재하면 "계획"이라고 표시한다.
- 실패하거나 결과가 좋지 않았던 사례도 제외하지 않는다.
...
- 충동 발생 시 중단 절차
- 디지털 기기·인터넷 사용 관리
- 음주·수면·스트레스 등 취약상태 관리
- 타인의 동의와 경계 존중을 위한 행동 기준
- 고립을 줄이는 안전한 관계와 도움 요청
- 상담자와 합의한 안전계획
- 문제행동으로 이어지는 사고의 점검

각 항목은 생각이나 다짐보다 실제 수행 행동을 우선한다.

## 6. 책임 인식과 자기성찰

### 자료에서 확인되는 자기인식

자신의 상황·생각·감정·충동·선택의 관계를 구체적으로 인식한 내용을 근거와 함께 쓴다.

### 행동 기준의 구체화

동의, 경계, 거리두기, 기기 사용, 도움 요청 등 앞으로 지켜야 할 행동 기준 중 자료에 존재하는 내용을 쓴다.

### 추가 탐색이 필요한 부분

책임의 외부화, 합리화, 축소, 회피 또는 불일치는 단정하지 말고 상담 질문으로 제시한다.

### 이번 주 감사 기록(참고자료)

AM-GRATITUDE-001의 representative_entries에 있는 원문을 날짜와 근거 ID를 붙여 그대로 제시한다. 타인에 대한 감사와 자신에 대한 감사를 구분한다. 이 항목에 대해 다음을 반드시 지킨다.

- 건수 자체를 "성실함" "진정성" 등으로 해석하지 않는다.
- 기록되지 않은 날이 있어도 감사할 일이 없었다고 단정하지 않는다(선택 입력 항목).
- 원문을 과장하거나 감정을 덧붙이지 않고 있는 그대로 인용한다.

| 날짜 | 구분(타인/자신) | 내용 | 근거 ID |
|---|---|---|---|

## 7. 미실천·무기록·지연 기록과 수정계획
...
미실천을 성과처럼 바꾸어 표현하지 않는다. 실패를 인정하고 수정 행동을 세운 사실이 있을 때만 그 과정을 별도로 쓴다.

## 8. 직전 주와의 비교

이전 자료가 있을 때만 작성한다.

| 항목 | 직전 주 | 이번 주 | 관찰된 차이 | 해석의 한계 | 근거 ID |
|---|---|---|---|---|---|
| 기록 지속성 | | | | | |
| 위험신호 조기 인식 | | | | | |
| 중단기술 사용 | | | | | |
...
수치 개선만으로 치료효과나 재범위험 감소를 결론내리지 않는다. 특히 감사 기록 건수는 AM-GRATITUDE-001.compared_to_previous_snapshot의 단순 수치 차이일 뿐이며, 선택 입력 항목이므로 건수가 늘거나 줄었다는 사실만 쓰고 "개선" "성실해짐" "동기가 높아짐" 같은 평가어를 붙이지 않는다.

## 9. 다음 주 실천계획

자료와 상담과제에 근거한 측정 가능한 행동목표를 3~5개 제시한다.

| 위험상황·시점 | 수행할 행동 | 빈도·기준 | 확인방법 | 도움 요청 기준 | 근거 |
|---|---|---|---|---|---|

AI가 새로운 치료과제를 임의로 처방하지 말고, 기존 과제와 기록에서 합리적으로 이어지는 초안임을 표시한다.

## 10. 사실 확인 구분표

| 주요 문장 또는 항목 | 출처 | 확인 상태 | 근거 ID |
|---|---|---|---|

내담자 자기보고, 상담자 직접 확인, 앱 계산, 채택·보류 해석을 구분한다.

## 11. 자료의 한계 및 상담자 검토사항

다음 취지를 반드시 포함한다.

"본 기록은 대상 기간에 제출된 자기관찰 자료와 상담기록을 토대로 작성된 AI 초안이다. 일상 행동의 상당 부분은 내담자의 자기보고에 근거하므로 독립적으로 확인된 사실과 구분하여 해석해야 한다. 기록의 존재만으로 반성의 진정성, 치료효과, 재범위험의 감소 또는 향후 행동을 단정할 수 없다. 외부 제출 전 상담자가 원자료 대조, 사실 확인, 비식별화 및 문장 수정을 완료해야 한다."
...
- 상담자가 확인할 핵심 질문:
- 외부 제출에서 제외할 내용:
- threshold_profile.personalized가 참이면: 이 문서가 이 내담자만의 개인화된 위험 판정 기준(정서·인지·충동 임계값)을 사용했다는 사실과, 일반 기준과 직접 비교할 수 없다는 점.
...
출력 전에 스스로 확인하고 위반사항을 수정하라.

1. JSON에 없는 사실·감정·행동을 만들지 않았는가?
2. 자기보고와 상담자 확인 사실을 구분했는가?
3. 실제 행동과 계획을 구분했는가?
4. 미실천·무기록·지연 기록을 숨기지 않았는가?
5. 날짜·상황·행동·결과·근거 ID가 연결되었는가?
6. 반성의 진정성, 완치, 재범위험 감소 또는 선처 필요성을 단정하지 않았는가?
7. 피해자와 제3자의 식별정보를 제거했는가?
8. 상담자가 확인할 불확실성을 별도로 표시했는가?
9. 감사 기록 건수나 변화를 "성실함" "진정성" "동기 향상" 등으로 해석하지 않고 사실로만 제시했는가?
10. 개인화된 임계값을 썼다면 그 사실과 일반 기준과 비교할 수 없다는 점을 명시했는가?
11. 관계적 영향을 다뤘다면, 상태 구분 외에 배우자·가족 등 제3자의 신원을 짐작할 수 있는 표현을 그대로 옮기지 않았는가?`;

const COMMON_PROFILE_RULES = `## 공통 근거 규칙

- 함께 제공된 maeumgoyo_ai_evidence_v1 JSON만 근거로 사용하라.
- 주요 사실과 해석 뒤에 근거 ID를 표시하라.
- SR은 내담자 자기보고, CO는 상담자 입력, AM은 앱 계산, AI는 보류된 해석, CH는 상담자가 채택한 해석으로 구분하라.
- 기록에 없는 사실·감정·행동·동기·피해 인식을 만들지 마라.
- 계획, 실제 수행, 부분 수행, 미수행을 구분하라.
- 무기록일, 지연 기록, 자료 부족, 불일치와 확인 한계를 숨기지 마라.
- 실명, 연락처, 기관명, 직장·학교, 구체적 장소 등 직접·간접 식별정보를 제거하라.
- 진단, 반성의 진정성, 완치, 재범위험 감소, 향후 행동, 양형 또는 선처 필요성을 단정하지 마라.
- AM 수치와 자동 연쇄는 상담자가 검토할 보조정보이지 임상적·법적 결론이 아니다.
- JSON에 safety_screening 필드가 있어도 그 내용(상태·메모)을 절대 인용하거나 요약하지 마라. 이 항목은 상담자의 세션 내 안전 확인용이며 어떤 목적의 문서에도 포함하지 않는다.
- 결과물 첫머리에 "AI 작성 초안 · 상담자 검토 필요"를 표시하라.`;

function makePromptProfile(title, task) {
  return `# ${title}\n\n${COMMON_PROFILE_RULES}\n\n${task}`;
}

const PROFILE_A_PROMPT = makePromptProfile("프로필 A — 50분 상담 회기준비안", `## 목적

상담자가 이번 회기를 준비할 수 있도록 최근 자료와 이전 임상맥락을 연결한 50분 회기안을 작성하라. AI가 치료를 지시하지 말고 상담자가 선택할 수 있는 초안을 제시하라.

## 출력 구조

1. 자료 범위와 품질: 기간, 기록일, 무기록일, 지연 기록, 상담자 확인 범위
2. 이번 회기 핵심 초점 1개와 보조 초점 최대 2개
3. 초점 선정 근거: SR·CO·AM을 구분하고 근거 ID 제시
4. 50분 시간표: 0~5분 안전·자료확인, 5~15분 피드백, 15~30분 핵심 연쇄, 30~40분 개입 연습, 40~47분 과제 합의, 47~50분 요약
5. 확인 질문: 사실확인, 경험탐색, 책임·경계, 변화언어 질문을 구분
6. 개입 후보: CBT·DBT·ACT·행동활성화 중 자료에 맞는 후보와 선택 이유
7. 회기 중 직접 확인해야 할 위험·불일치·자료 공백
8. 다음 주 작은 실천: 상황, 행동, 빈도, 확인방법, 도움요청 기준
9. 상담자가 채택·수정·보류할 결정표

자해·타해 또는 급성 위험이 자료에서 의심되면 일반 회기안보다 상담자의 직접 위험평가와 기관 절차를 우선하라고 표시하라.`);

const PROFILE_B_PROMPT = makePromptProfile("프로필 B — 상세 사례분석", `## 목적

상담자의 사례개념화를 돕는 상세 분석 초안을 작성하라. 진단이나 공식 재범위험성 평가를 대신하지 않는다.

## 출력 구조

1. 자료 개요와 신뢰도 한계
2. 현재 어려움과 치료 목표
3. 4P 사례개념화: 취약요인, 촉발요인, 유지요인, 보호요인. 각 항목을 사실·가설로 구분
4. 기능분석: 상황 → 생각 → 감정·신체 → 충동 → 행동 → 단기결과 → 장기결과 → 대안행동
5. 반복되는 위험 연쇄와 예외·부분성공 장면
6. 자기관찰, 감정감내, 중단기술, 대안행동, 도움요청의 변화
7. 실천행동의 계획 대비 수행과 장애요인
8. 예기불안과 실제 결과의 차이
9. 책임 인식·동의·경계·피해예방 관점에서 추가 탐색할 질문
10. 강점과 보호자원
11. 단기·중기 개입 후보와 근거
12. 다음 회기에서 검증할 가설과 필요한 추가자료

공식 위험평가 도구를 사용하지 않은 경우 위험 수준을 낮음·중간·높음으로 분류하지 마라.`);

const PROFILE_C_PROMPT = makePromptProfile("프로필 C — 다음 회기 임상맥락", `## 목적

이번 회기의 핵심 내용만 다음 상담자·다음 AI 작업에 이어질 구조화된 임상맥락으로 정리하라. 긴 분석 보고서를 반복하지 마라.

## 출력 구조

1. 프로젝트·자료기간·회기일
2. 이번 회기에서 확인된 사실
3. 상담자가 채택한 해석·개입과 근거
4. 보류하거나 기각한 AI 가설
5. 반복 연쇄와 새롭게 발견된 예외
6. 과제 수행 결과와 장애요인
7. 자료 공백·불일치·안전 확인사항
8. 다음 회기에서 반드시 확인할 질문 3~7개
9. 다음 주 측정·기록 초점
10. 다음 AI가 유지해야 할 맥락과 피해야 할 단정

CH와 AI를 혼동하지 말고, 상담자가 채택하지 않은 가설은 반드시 보류 상태로 남겨라.`);

const PROFILE_D_PROMPT = makePromptProfile("프로필 D — 자기성찰 질문", `## 목적

내담자가 자신의 기록을 다시 살펴보고 스스로 답할 수 있는 구체적인 자기성찰 질문을 작성하라. 답변이나 반성문을 대신 작성하지 마라.

## 출력 구조

1. 사실 확인 질문 2~3개
2. 상황·생각·감정·충동·행동 연결 질문 3~4개
3. 책임·동의·경계·타인의 안전을 점검하는 질문 2~3개
4. 중단기술·대안행동·도움요청을 강화하는 질문 2~3개
5. 실패·미실천·무기록 이후 수정계획 질문 2개
6. 다음 주 가치기반 행동 질문 1~2개

각 질문 뒤에 질문의 근거 ID를 표시하라. 자백, 자기비난 또는 특정한 법적 결론을 유도하지 말고 개방형 질문을 사용하라.`);

const PROFILE_E_PROMPT = makePromptProfile("프로필 E — 예비 자기성찰문 작성 보조", `## 목적

내담자가 자신의 언어로 다시 작성할 수 있는 예비 자기성찰문 초안을 제공하라. 법원 제출용 최종 반성문을 대필하지 마라.

## 출력 규칙

- 첫 문장에 "아래 글은 기록을 정리한 예비 초안이며 본인이 사실관계를 확인하고 자기 언어로 다시 작성해야 합니다"라고 표시한다.
- 1인칭을 사용할 수 있지만 기록에 없는 감정·사과·다짐·피해 인식을 만들지 않는다.
- 구체적 상황, 당시 생각과 감정, 선택한 행동, 결과, 새롭게 알게 된 점, 다음 행동 기준 순서로 쓴다.
- 잘한 행동만 고르지 말고 미실천·회피·기록 공백과 수정계획을 균형 있게 포함한다.
- 상투적인 사죄, 과도한 자기비난, 선처 호소를 사용하지 않는다.
- 각 문단 끝에 사용한 근거 ID를 표시한다.
- A4 반 페이지 안팎으로 작성한다.

끝에 내담자가 직접 답하고 수정할 확인 질문 5개를 덧붙여라.`);

const PROFILE_G_PROMPT = makePromptProfile("프로필 G — 월간 치료과정 요약", `## 목적

여러 주의 표준 근거자료를 바탕으로 한 달 동안의 치료참여와 변화 노력을 종합하라. 주간 기록을 단순 연결하지 말고 지속성·변동·반복 패턴을 보여라.

## 출력 구조

1. 대상기간, 포함 주차, 회기 참여와 자료 완전성
2. 월간 치료목표와 주차별 핵심과제
3. 자기관찰 지속성: 기록일·무기록일·지연 기록의 주차별 변화
4. 위험신호 조기 인식과 반복 연쇄
5. 중단기술·대안행동·도움요청의 구체적 사례
6. 상담과제의 계획·수행·부분수행·미수행
7. 책임·동의·경계·피해예방 학습의 구체화
8. 부분성공, 후퇴, 실패 후 수정 과정
9. 앱 수치 변화표와 해석의 한계
10. 상담자가 직접 확인한 사실과 자기보고 분리표
11. 다음 달 치료 초점과 측정 가능한 실천계획
12. 자료 한계와 상담자 검토란

주차 간 자료량이 다르면 단순 합계나 평균을 성과처럼 비교하지 말고 자료 밀도의 차이를 명시하라.`);

const PROFILE_H_PROMPT = makePromptProfile("프로필 H — 상담자 의견서 근거개요", `## 목적

상담자가 치료참여 확인서·치료경과 의견서 등을 직접 작성하기 전에 사용할 근거개요를 만든다. 최종 의견서, 법정신의학 감정, 공식 위험평가 또는 양형 의견을 대신 작성하지 않는다.

## 출력 구조

1. 문서 작성 목적과 사용 가능한 자료 범위
2. 상담자가 직접 확인한 사실: 회기일, 참석, 회기 중 수행, 제출물 확인
3. 내담자 자기보고에만 근거한 일상 실천
4. 앱이 계산한 보조수치와 자동 연쇄
5. 구체적 치료참여 노력의 연대표
6. 과제 수행, 중단기술, 대안행동, 도움요청의 근거표
7. 미실천·무기록·지연 기록·불일치
8. 책임·동의·경계·피해예방과 관련해 상담에서 다룬 내용
9. 상담자가 추가 확인해야 할 사실과 질문
10. 의견서에 사용할 수 있는 중립적 문장 후보와 각 근거 ID
11. 의견서에서 제외해야 할 미확인 주장
12. 자료 및 판단의 한계

중립적 문장 후보에도 자기보고와 직접 확인을 명시하라. 치료효과, 재범위험, 진정성, 선처 또는 형량에 관한 결론 문장은 생성하지 마라.`);

const PROMPT_PROFILES = {
  A: { code: "A", title: "50분 상담 회기준비안", description: "이번 50분 회기의 초점·질문·개입 후보·과제를 구성합니다.", prompt: PROFILE_A_PROMPT },
  B: { code: "B", title: "상세 사례분석", description: "4P 사례개념화와 기능분석을 포함한 상담자용 분석 초안을 만듭니다.", prompt: PROFILE_B_PROMPT },
  C: { code: "C", title: "다음 회기 임상맥락", description: "채택·보류 가설과 다음 확인점을 짧고 구조적으로 이어갑니다.", prompt: PROFILE_C_PROMPT },
  D: { code: "D", title: "자기성찰 질문", description: "내담자가 스스로 답할 근거기반 질문만 생성합니다.", prompt: PROFILE_D_PROMPT },
  E: { code: "E", title: "예비 자기성찰문", description: "내담자가 자기 언어로 다시 쓸 출발점이며 최종 반성문이 아닙니다.", prompt: PROFILE_E_PROMPT },
  F: { code: "F", title: "주간 치료참여 및 재범방지 실천기록", description: "매주의 구체적 변화 노력과 미실천·수정과정을 법원 제출용 초안 형식으로 정리합니다.", prompt: PROFILE_F_PROMPT },
  G: { code: "G", title: "월간 치료과정 요약", description: "여러 주 자료에서 지속성·변동·반복 패턴과 다음 달 과제를 정리합니다.", prompt: PROFILE_G_PROMPT },
  H: { code: "H", title: "상담자 의견서 근거개요", description: "상담자가 최종 의견서를 작성하기 전 확인된 사실과 미확인 주장을 분리합니다.", prompt: PROFILE_H_PROMPT },
};

function getSelectedPromptProfile() {
  return PROMPT_PROFILES[els.promptProfileSelect?.value || "F"] || PROMPT_PROFILES.F;
}

function renderPromptProfileDescription() {
  if (!els.promptProfileDescription) return;
  const profile = getSelectedPromptProfile();
  els.promptProfileDescription.textContent = `프로필 ${profile.code}: ${profile.description}`;
  if (els.promptReadinessResult) {
    els.promptReadinessResult.className = "preflight-result";
    els.promptReadinessResult.textContent = "선택한 프로필의 준비상태를 다시 점검하세요.";
  }
}

function copySelectedPromptProfile() {
  const profile = getSelectedPromptProfile();
  copyTextToClipboard(profile.prompt, `프로필 ${profile.code} 프롬프트를 복사했습니다. AI 표준 근거자료 JSON과 함께 사용하세요.`);
}

function saveSelectedPromptProfile() {
  const profile = getSelectedPromptProfile();
  downloadText(buildPromptProfileFileName(profile), profile.prompt, "text/markdown;charset=utf-8", els.saveSelectedPromptBtn);
}

function runSelectedPromptReadiness() {
  const profile = getSelectedPromptProfile();
  if (!state.rows.length) {
    renderPreflightResult(els.promptReadinessResult, { status: "invalid", errors: ["CSV를 먼저 불러오세요."], warnings: [], checks: [{ name: "csv_required", passed: false, severity: "error", message: "CSV를 먼저 불러오세요." }] }, `프로필 ${profile.code}`, { askToMove: true });
    return;
  }
  const packageData = buildAiEvidencePackage();
  const readiness = validatePromptProfileReadiness(profile, packageData);
  renderPreflightResult(els.promptReadinessResult, readiness, `프로필 ${profile.code}`, { askToMove: true });
}

function validatePromptProfileReadiness(profile, packageData) {
  const errors = [...packageData.validation.errors];
  const warnings = [];
  const checks = [];
  const add = (name, passed, severity, message) => {
    checks.push({ name, passed, severity, message });
    if (!passed) (severity === "error" ? errors : warnings).push(message);
  };
  const recordTypes = packageData.source_integrity.record_type_counts || {};
  const hasVerifiedFact = packageData.counselor_evidence.some((item) => item.evidence_type === "directly_verified_fact");
  const hasVerificationLimit = packageData.counselor_evidence.some((item) => item.evidence_type === "verification_limit");
  const hasAssignedTask = packageData.counselor_evidence.some((item) => item.evidence_type === "assigned_task");
  const hasSessionDate = Boolean(packageData.treatment_participation.session_date);
  const hasAttendance = packageData.treatment_participation.attendance_status !== "not_recorded";

  add("privacy_review", packageData.privacy.potential_identifier_findings.length === 0, "warning", `개인정보 형식 후보 ${packageData.privacy.potential_identifier_findings.length}곳을 검토해야 합니다.`);
  add("usable_daily_evidence", (recordTypes.observation || 0) + (recordTypes.practice_log || 0) + (recordTypes.daily_checkin || 0) > 0, "error", "날짜별 관찰·실천·하루회고 근거가 없습니다.");

  if (["A", "B"].includes(profile.code)) {
    add("derived_chain", packageData.derived_chains.length > 0, "warning", "분석할 자동 연쇄가 없습니다. 원기록 중심으로 제한적으로 작성해야 합니다.");
  }
  if (profile.code === "C") {
    add("clinical_continuity", packageData.clinical_continuity.length > 0 || packageData.counselor_evidence.length > 0, "warning", "이어갈 상담자 맥락이나 상담 메모가 없습니다.");
  }
  if (["D", "E"].includes(profile.code)) {
    add("observation_evidence", (recordTypes.observation || 0) > 0, "error", "자기성찰 질문·초안의 근거가 될 observation 기록이 없습니다.");
  }
  if (profile.code === "F") {
    add("session_date", hasSessionDate, "error", "프로필 F에는 상담 회기일이 필요합니다.");
    add("attendance", hasAttendance, "error", "프로필 F에는 회기 참여 상태가 필요합니다.");
    add("verified_fact", hasVerifiedFact, "error", "프로필 F에는 상담자가 직접 확인한 사실이 최소 1건 필요합니다.");
    add("assigned_task", hasAssignedTask, "warning", "이번 주 상담과제가 입력되지 않았습니다.");
    add("verification_limit", hasVerificationLimit, "warning", "자기보고와 직접 확인의 범위를 설명할 확인 한계가 입력되지 않았습니다.");
  }
  if (profile.code === "G") {
    add("monthly_span", packageData.period_and_completeness.calendar_span_days >= 21, "error", "월간 요약에는 최소 21일 범위의 자료를 권장합니다.");
    add("weekly_comparison", packageData.comparison.previous_snapshot_available || packageData.source_integrity.source_files.length >= 2, "warning", "주차 간 비교를 확인할 이전 요약 또는 여러 원본 파일 정보가 없습니다.");
  }
  if (profile.code === "H") {
    add("session_date", hasSessionDate, "error", "의견서 근거개요에는 상담 회기일이 필요합니다.");
    add("attendance", hasAttendance, "error", "의견서 근거개요에는 회기 참여 상태가 필요합니다.");
    add("verified_fact", hasVerifiedFact, "error", "의견서 근거개요에는 상담자가 직접 확인한 사실이 필요합니다.");
    add("verification_limit", hasVerificationLimit, "error", "의견서 근거개요에는 확인 범위와 한계가 필요합니다.");
  }

  packageData.validation.warnings.forEach((warning) => {
    if (!warnings.includes(warning)) warnings.push(warning);
  });
  const uniqueErrors = [...new Set(errors)];
  const uniqueWarnings = [...new Set(warnings)];
  return {
    validator_version: "maeumgoyo_prompt_readiness_v1",
    profile_code: profile.code,
    status: uniqueErrors.length ? "invalid" : uniqueWarnings.length ? "valid_with_warnings" : "valid",
    errors: uniqueErrors,
...
function formatObservationBlock(payload, index, total) {
  ...
  return [
    ...
    behaviorAreas ? `  경험한 문제: ${behaviorAreas}` : "",
...
function extractDailyReflectionSummaries(rows, limit = REFLECTION_DAY_LIMIT) {
  ...
  return recentDates.map((date) => {
    ...
    if (dayCheckin) {
      ...
      const others = normalizeGratitudeList(dayCheckin.payload.gratitude_others);
      const self = normalizeGratitudeList(dayCheckin.payload.gratitude_self);
      if (others.length || self.length) {
        sections.push("  감사 기록:");
        others.forEach((text) => sections.push(`  - (타인에게) ${text}`));
        self.forEach((text) => sections.push(`  - (자신에게) ${text}`));
      }
      sections.push("");
    }
...
function buildAiReportSection2(observations) {
  ...
  return observations.map(({ row, payload }) => {
    ...
    if (behaviorText) lines.push(`  경험한 문제: ${behaviorText}`);
    ...
    lines.push(`  문제행동수준: ${payload.action_level ?? "-"}/5`);
...
// --- 개인별 임계값 보정 ---
// 이 내담자 자신의 지난 자기보고 분포에서 임계값 제안을 계산합니다. 절대 자동 적용되지 않고,
// 상담자가 "개인화 제안 적용" 버튼을 눌러야만 활성화됩니다(applyPersonalizedThresholds 참고).
// 문제행동수준(action) 임계값은 "실제 행동화 여부"를 판정하는 안전망이라 개인화 대상에서 제외합니다.
const THRESHOLD_CALIBRATION_MIN_N = 10;
const THRESHOLD_CALIBRATION_RECALIBRATE_DAYS = 28; // 4주
...
// 변별력 검사: 값이 거의 다 같으면(사분위범위가 1점 이하) 개인화 임계값이 사실상 무의미해지므로 보류합니다.
function calibrateOneMetric(values, { highPercentile, lowFloor, highCeiling }) {
  const sorted = values.filter((v) => v !== null && Number.isFinite(v)).sort((a, b) => a - b);
  const n = sorted.length;
  if (n < THRESHOLD_CALIBRATION_MIN_N) {
    return { sufficient: false, n, reason: `자료 부족(관찰 ${n}건, 최소 ${THRESHOLD_CALIBRATION_MIN_N}건 필요)` };
  }
  const p25 = percentile(sorted, 25);
  const p75 = percentile(sorted, 75);
  if (p75 - p25 <= 1) {
    return { sufficient: false, n, reason: "값이 대부분 비슷해 변별력이 부족함(개인화 보류)" };
  }
  const median = percentile(sorted, 50);
  const highRaw = percentile(sorted, highPercentile);
  const high = Math.round(clamp(highRaw, lowFloor, highCeiling, highRaw));
  return { sufficient: true, n, median: Math.round(median * 10) / 10, p75: Math.round(p75 * 10) / 10, value: high };
}

function computeThresholdCalibration() {
  const urgeValues = state.observationDays.map((d) => d.urgeScore);
  const emotionValues = state.observationDays.map((d) => d.emotionScore);

  const urge = calibrateOneMetric(urgeValues, { highPercentile: 75, lowFloor: 2, highCeiling: 10 });
  const emotion = calibrateOneMetric(emotionValues, { highPercentile: 75, lowFloor: 2, highCeiling: 10 });
  // 인지 주의(cognitiveUrge)는 원래도 urge보다 낮은, 더 이른 경고선이라 같은 충동 분포의 중앙값(50백분위)을 씁니다.
  let cognitiveUrge = calibrateOneMetric(urgeValues, { highPercentile: 50, lowFloor: 1, highCeiling: 9 });
  if (urge.sufficient && cognitiveUrge.sufficient && cognitiveUrge.value >= urge.value) {
    cognitiveUrge = { ...cognitiveUrge, value: Math.max(1, urge.value - 1) };
  }

  return { urge, emotion, cognitiveUrge, computedAt: new Date().toISOString() };
}

function isThresholdProfileStale(profile) {
  if (!profile?.appliedAt) return false;
  const appliedMs = new Date(profile.appliedAt).getTime();
  if (!Number.isFinite(appliedMs)) return false;
  const days = (Date.now() - appliedMs) / (1000 * 60 * 60 * 24);
  return days >= THRESHOLD_CALIBRATION_RECALIBRATE_DAYS;
}

function applyPersonalizedThresholds() {
  const calibration = computeThresholdCalibration();
  if (!calibration.urge.sufficient && !calibration.emotion.sufficient && !calibration.cognitiveUrge.sufficient) {
    showActionToast("개인화할 수 있는 자료가 아직 부족합니다.", "warning");
    return;
  }
  const before = { ...state.thresholds };
  if (calibration.urge.sufficient) state.thresholds.urge = calibration.urge.value;
  if (calibration.emotion.sufficient) state.thresholds.emotion = calibration.emotion.value;
  if (calibration.cognitiveUrge.sufficient) state.thresholds.cognitiveUrge = calibration.cognitiveUrge.value;
  ...
  state.thresholdProfile = {
    ...
    confirmedBy: "counselor",
    basis: {
      urge: calibration.urge,
      emotion: calibration.emotion,
      cognitiveUrge: calibration.cognitiveUrge,
    },
    values: { ...state.thresholds },
    genericDefaults: { emotion: 7, cognitiveUrge: 6, urge: 8, action: 4 },
  };

  showActionToast(
    `개인화 임계값을 적용했습니다: 충동 ${before.urge}→${state.thresholds.urge}, 정서 ${before.emotion}→${state.thresholds.emotion}, 인지주의 ${before.cognitiveUrge}→${state.thresholds.cognitiveUrge}`,
    "success",
  );
  render();
}

function renderThresholdCalibration() {
  if (!els.thresholdCalibrationSuggestion) return;
  if (!state.records.length) {
    els.thresholdCalibrationSuggestion.textContent = "CSV를 불러오면 개인화 제안이 계산됩니다.";
    els.applyPersonalizedThresholdsBtn.hidden = true;
  } else {
    const c = computeThresholdCalibration();
    const lines = [];
    if (c.urge.sufficient) lines.push(`충동 ${state.thresholds.urge}→${c.urge.value} (관찰 ${c.urge.n}건, 중앙값 ${c.urge.median}, 75백분위 ${c.urge.p75})`);
    else lines.push(`충동: ${c.urge.reason}`);
    if (c.emotion.sufficient) lines.push(`정서 ${state.thresholds.emotion}→${c.emotion.value} (관찰 ${c.emotion.n}건, 중앙값 ${c.emotion.median}, 75백분위 ${c.emotion.p75})`);
    else lines.push(`정서: ${c.emotion.reason}`);
    if (c.cognitiveUrge.sufficient) lines.push(`인지주의 ${state.thresholds.cognitiveUrge}→${c.cognitiveUrge.value} (같은 충동 분포의 중앙값 기준)`);
    else lines.push(`인지주의: ${c.cognitiveUrge.reason}`);
    els.thresholdCalibrationSuggestion.innerHTML = lines.map((line) => `<div>${escapeHtml(line)}</div>`).join("");
    els.applyPersonalizedThresholdsBtn.hidden = !(c.urge.sufficient || c.emotion.sufficient || c.cognitiveUrge.sufficient);
  }

  if (els.thresholdProfileStatus) {
    if (!state.thresholdProfile) {
      els.thresholdProfileStatus.textContent = "적용된 개인화 임계값이 없습니다(일반 기준 사용 중).";
    } else {
      const stale = isThresholdProfileStale(state.thresholdProfile);
      const overridden = ["urge", "emotion", "cognitiveUrge"].some(
        (key) => state.thresholds[key] !== state.thresholdProfile.values[key],
      );
      const parts = [`개인화 임계값 적용됨 (${new Date(state.thresholdProfile.appliedAt).toLocaleDateString("ko-KR")})`];
      if (stale) parts.push("4주 이상 지나 재보정을 권장합니다.");
      if (overridden) parts.push("적용 이후 값이 수동으로 변경되어 현재 화면 기준과 다를 수 있습니다.");
      els.thresholdProfileStatus.textContent = parts.join(" ");
    }
  }
}
...
function computeGoalProgress() {
  ...
  return [
    ...
    { label: "문제행동수준 평균(낮을수록 좋음)", actual: avgAction, goal: actionGoal, unit: "", higherIsBetter: false },
...
function renderSessionFeedbackSummaryView() {
  ...
  return visual + workbench([
    ...
    ["다음 주 계획", [buildGoalProgressCard(), card("회기 통합·다음 주 실험 설계", `<p>이번 회기 비교용 요약 파일에는 원본 CSV가 저장되지 않고 현재 분석 요약과 상담 메모만 저장됩니다.</p>`, "focus", [
...
async function copyTextToClipboard(text, successMessage) {
  try {
    ...
    const message = `${successMessage} 민감정보가 포함될 수 있으니 필요한 곳에만 붙여넣으세요.`;
    showImportMessages([message]);
    showActionMessage(message);
  } catch (error) {
    const message = "복사하지 못했습니다. 브라우저의 클립보드 권한을 허용한 뒤 다시 시도하세요.";
    showImportMessages([message]);
...
// "이번 회기 비교용 요약" JSON 파일을 다음 회기에 다시 불러와 이번 주와 비교합니다. 오늘 이후 저장되는 요약에는
...
function computeCurrentWeeklySnapshot() {
  ...
  const gratitude = computeGratitudeMeasures();
  ...
  return {
    ...
    // 감사 건수: 삶의 경험 확장/만족도(avgExpansion)와 같은 카드에서 나오는 자료라 스냅샷에 함께 보존합니다.
    // SNAPSHOT_METRICS에는 의도적으로 넣지 않습니다(개선/악화 자동 라벨링 대상 제외 — 선택 입력 항목이므로).
    gratitudeOthersCount: gratitude.othersCount,
    gratitudeSelfCount: gratitude.selfCount,
    gratitudeRecordedDays: gratitude.daysWithAnyGratitude,
...
function extractSnapshotFromSummaryJson(payload) {
  ...
  if (payload.weeklySnapshot) {
    return { ...payload.weeklySnapshot, thresholdProfile: payload.thresholdProfile || null, isLegacy: false };
  }
...
function renderSessionComparison() {
  ...
  if (!state.previousSnapshot) {
    els.sessionComparisonResult.innerHTML = `<p class="muted">비교할 지난 회기 요약 파일을 아직 불러오지 않았습니다.</p>`;
    return;
  }
  if (!state.records.length) {
  ...
  const items = SNAPSHOT_METRICS.map(([key, label, unit, higherIsBetter]) => {
    ...
    if (prevVal === null || prevVal === undefined) return `<li>${label}: 지난 회기 요약에 없음 (이번 주 ${currVal !== null && currVal !== undefined ? currVal.toFixed(1) + unit : "기록 없음"})</li>`;
  ...
  const legacyNote = prev.isLegacy
    ? `<p class="muted">지난 회기 요약이 이번 비교 기능 도입 이전 형식이라, 경험 확장·변화언어 비율은 비교할 수 없습니다.</p>`
    : "";

  // 감사 건수는 선택 입력 항목이라 SNAPSHOT_METRICS의 개선/악화 판정에 포함하지 않고,
  // 여기서 숫자 변화만 별도로 중립적으로 보여줍니다.
  const gratitudeLine = (Number.isFinite(prev.gratitudeOthersCount) || Number.isFinite(prev.gratitudeSelfCount))
    ? `<li>감사 기록(타인 ${prev.gratitudeOthersCount ?? "-"}→${curr.gratitudeOthersCount ?? "-"}건, 자신 ${prev.gratitudeSelfCount ?? "-"}→${curr.gratitudeSelfCount ?? "-"}건) — 선택 입력 항목이므로 건수 변화를 개선·악화로 판단하지 않습니다.</li>`
    : `<li>감사 기록: 이번 주 타인 ${curr.gratitudeOthersCount ?? 0}건 · 자신 ${curr.gratitudeSelfCount ?? 0}건 (지난 회기 요약에 값 없음)</li>`;
  ...
  els.sessionComparisonResult.innerHTML = `
    ...
    <ul>${items}${gratitudeLine}</ul>
    ${legacyNote}
  `;
}

function exportAiEvidencePackage() {
  if (!state.rows.length) {
    showSaveProblem("저장할 AI 표준 근거자료가 없습니다. CSV를 먼저 불러오세요.");
    return;
  }
  const packageData = buildAiEvidencePackage();
  renderPreflightResult(els.evidencePreflightResult, packageData.validation, "표준 근거자료");
  if (packageData.validation.errors.length) {
    showSaveProblem(`표준 근거자료 오류 ${packageData.validation.errors.length}건을 먼저 수정하세요. 저장하지 않았습니다.`);
    return;
  }
  const warning = packageData.validation.warnings.length
    ? `사전점검 경고가 ${packageData.validation.warnings.length}건 있습니다. 경고와 개인정보 후보를 검토하고 비식별 상태를 직접 확인했습니까?`
    : "사전점검 오류는 없습니다. 실명·기관명·구체적 장소 등 자동 탐지가 어려운 식별정보를 직접 확인했습니까?";
  if (!window.confirm(warning)) return;
  downloadText(buildAiEvidenceFileName(), JSON.stringify(packageData, null, 2), "application/json;charset=utf-8", els.aiEvidenceExportBtn);
}

function runEvidencePreflight() {
  if (!state.rows.length) {
    renderPreflightResult(els.evidencePreflightResult, { status: "invalid", errors: ["CSV를 먼저 불러오세요."], warnings: [], checks: [{ name: "csv_required", passed: false, severity: "error", message: "CSV를 먼저 불러오세요." }] }, "표준 근거자료", { askToMove: true });
    return;
  }
  const packageData = buildAiEvidencePackage();
  renderPreflightResult(els.evidencePreflightResult, packageData.validation, "표준 근거자료", { askToMove: true });
}

function buildAiEvidencePackage() {
  const usableRows = filterUsableRows(state.rows);
  const sourceEvidence = usableRows.map((row) => buildSourceEvidenceItem(row));
  const dateCoverage = computeEvidenceDateCoverage(usableRows);
  const delayedEntries = sourceEvidence
    .filter((item) => item.entry_timing.days_after_record_date !== null && item.entry_timing.days_after_record_date >= 2)
    .map((item) => ({
      evidence_id: item.evidence_id,
      record_date: item.date,
      updated_at: item.source_metadata.updated_at,
      days_after_record_date: item.entry_timing.days_after_record_date,
    }));
  const parseFailures = sourceEvidence.filter((item) => item.content === null).map((item) => item.evidence_id);
  const piiFindings = [];
  sourceEvidence.forEach((item) => scanPotentialPersonalIdentifiers(item.content, item.evidence_id, "content", piiFindings));
  scanPotentialPersonalIdentifiers(els.notes?.value, "CO-NOTE-001", "text", piiFindings);
  scanPotentialPersonalIdentifiers(els.assignedTasksInput?.value, "CO-TASK-001", "text", piiFindings);
  scanPotentialPersonalIdentifiers(els.counselorVerifiedInput?.value, "CO-VERIFY-001", "text", piiFindings);
  scanPotentialPersonalIdentifiers(els.verificationLimitsInput?.value, "CO-LIMIT-001", "text", piiFindings);
  scanPotentialPersonalIdentifiers(els.interventionBranchNote?.value, "CO-BRANCH-001", "text", piiFindings);
  scanPotentialPersonalIdentifiers(els.relationalImpactNote?.value, "CO-RELATIONAL-NOTE-001", "text", piiFindings);
  scanPotentialPersonalIdentifiers(els.safetyScreeningNote?.value, "CO-SAFETY-001", "text", piiFindings);
  scanPotentialPersonalIdentifiers(state.previousAiContextText, "CO-CONTEXT-001", "text", piiFindings);
  scanPotentialPersonalIdentifiers(els.acceptedAiSuggestions?.value, "CH-001", "text", piiFindings);
  scanPotentialPersonalIdentifiers(els.deferredAiSuggestions?.value, "AI-DEFERRED-001", "text", piiFindings);
  scanPotentialPersonalIdentifiers(getProjectName(), "PACKAGE-METADATA", "project_name", piiFindings);
  (state.fileNames.length ? state.fileNames : [state.fileName]).forEach((name, index) => {
    scanPotentialPersonalIdentifiers(name, "PACKAGE-METADATA", `source_files[${index}]`, piiFindings);
  });

  const predictionStats = predictionAccuracyStats(state.predictions);
  const practiceGroups = groupPractice();
  const focus = assessWeekFocus();
  const currentSnapshot = computeCurrentWeeklySnapshot();
  const previousSnapshot = state.previousSnapshot ? { ...state.previousSnapshot } : null;
  const gratitudeMeasures = computeGratitudeMeasures();
  const counselorEvidence = buildCounselorEvidenceItems();
  const aiContinuity = buildContinuityEvidenceItems();

  const packageData = {
    schema_version: AI_EVIDENCE_SCHEMA,
    package_metadata: {
      package_id: `MGAI-${getClientAlias()}-${computeDataDateRangeLabel()}-${Date.now()}`,
      app: "마음고요 상담분석실",
      app_version: APP_VERSION,
      created_at: new Date().toISOString(),
      document_role: "목적별 AI 문서 생성에 공통으로 사용하는 근거자료 패키지",
      contains_task_prompt: false,
      subject_alias: getClientAlias(),
    ...
    provenance_legend: {
      SR: "내담자 자기보고 또는 내담자 앱 입력",
      AM: "앱이 원자료에서 계산한 값",
      CO: "상담자가 직접 입력한 관찰·확인 사실",
      AI: "AI가 생성했으나 상담자가 채택하지 않은 해석",
      CH: "상담자가 검토하여 채택한 AI 제안 또는 해석",
    },
    source_integrity: {
      source_files: state.fileNames.length ? state.fileNames : [state.fileName].filter(Boolean),
      source_schema: CSV_SCHEMA,
      share_modes: unique(state.rows.map((row) => row.share_mode).filter(Boolean)),
      imported_row_count: state.rows.length,
      eligible_row_count: usableRows.length,
      excluded_row_count: state.rows.length - usableRows.length,
      record_type_counts: countBy(usableRows, (row) => row.record_type || "unknown"),
      duplicate_policy: "같은 id는 updated_at이 가장 최신인 행만 앱 적재 단계에서 유지",
      payload_parse_failure_evidence_ids: parseFailures,
    },
    period_and_completeness: {
      app_range_label: state.rangeLabel || null,
      app_range_days: state.rangeDays,
      first_record_date: dateCoverage.first,
      last_record_date: dateCoverage.last,
      calendar_span_days: dateCoverage.spanDays,
      days_with_any_daily_record: dateCoverage.recordedDays.length,
      dates_with_any_daily_record: dateCoverage.recordedDays,
      dates_without_daily_record_inside_observed_span: dateCoverage.missingDays,
      delayed_entries_two_or_more_days: delayedEntries,
      limitation: "무기록일은 관찰된 최초일과 최종일 사이에서만 계산됩니다. 무기록이 곧 미실천을 뜻하지 않으며, 지연 작성도 updated_at과 기록일의 차이에 근거한 참고값입니다.",
    },
    privacy: {
      deidentification_status: "counselor_review_required",
      potential_identifier_findings: piiFindings,
      automatic_scan_scope: ["이메일 형식", "전화번호 형식", "주민등록번호 형식", "웹 주소"],
      automatic_scan_limitations: "실명, 별칭, 기관명, 직장·학교, 구체적 장소와 서술 맥락에 포함된 간접식별자는 자동으로 완전하게 찾을 수 없습니다.",
    },
    treatment_participation: {
      session_date: els.sessionDateInput?.value || null,
      attendance_status: els.sessionAttendanceInput?.value || "not_recorded",
      assigned_tasks: textOrNull(els.assignedTasksInput?.value),
      verification_limits: textOrNull(els.verificationLimitsInput?.value),
      intervention_branch_rationale: textOrNull(els.interventionBranchNote?.value),
      evidence_refs: counselorEvidence.map((item) => item.evidence_id),
    },
    // 관계적 영향(손상·회복): 성중독뿐 아니라 도박·알코올 등 가족 신뢰 손상이 흔한 모든 사례에 공통으로 쓰는
    // 선택 입력 축입니다. 상태 구분(범주)만 구조화하고, 제3자 신원 정보는 자유 텍스트에도 담기지 않도록
    // 화면에서부터 경고합니다. 상담자 관찰이지 내담자 자기보고 검증이 아니므로 provenance는 CO입니다.
    relational_impact: {
      provenance: "CO",
      disclosure_status: els.relationalDisclosureStatus?.value || "not_recorded",
      disclosure_status_label: RELATIONAL_DISCLOSURE_LABELS[els.relationalDisclosureStatus?.value] || "기록하지 않음",
      repair_status: els.relationalRepairStatus?.value || "not_recorded",
      repair_status_label: RELATIONAL_REPAIR_LABELS[els.relationalRepairStatus?.value] || "기록하지 않음",
      counselor_note_evidence_id: textOrNull(els.relationalImpactNote?.value) ? "CO-RELATIONAL-NOTE-001" : null,
      limitation: "이 항목은 상담자의 관찰이며 내담자 자기보고나 독립적 확인을 대체하지 않습니다. 제3자(배우자·가족 등)의 신원을 짐작할 수 있는 정보는 포함되어서는 안 됩니다.",
    },
    // 자살·자해 위험 선별: 표준화된 평가 도구가 아니라 상담자가 매 회기 직접 확인하는 3단계 선별입니다.
    // 재발위기 자동 초점 판정(automatic_focus)과 완전히 별개이며, 화면에도 별도의 최상단 배너로 뜹니다.
    // 이 항목은 세션 내 안전 확인용이며, F/G/H 등 외부 제출 문서에는 절대 포함하지 않습니다.
    safety_screening: {
      provenance: "CO",
      status: els.safetyScreeningStatus?.value || "not_recorded",
      status_label: SAFETY_SCREENING_LABELS[els.safetyScreeningStatus?.value] || "기록하지 않음",
      note_evidence_id: textOrNull(els.safetyScreeningNote?.value) ? "CO-SAFETY-001" : null,
      persists_across_sessions: false,
      limitation: "이 항목은 표준화된 위험도 평가 도구가 아니라 상담자의 선별 판단이며, 자살위험 진단이나 평가를 대체하지 않습니다. 매 회기 새로 확인해야 하며 이전 회기 값이 자동으로 이어지지 않습니다. 이 항목은 내부 안전 확인용이며 F/G/H 등 외부 제출 문서에는 포함하지 않습니다.",
    },
    current_period_app_measures: {
      provenance: "AM",
      evidence_id: "AM-SNAPSHOT-001",
      weekly_snapshot: currentSnapshot,
      counts: {
        records: state.records.length,
        chains: state.chains.length,
        high_risk_chains: chains((chain) => chain.highUrge && chain.actionized).length,
        controlled_chains: chains(isControlled).length,
        partial_success_chains: chains(isPartialSuccess).length,
        distress_tolerance_chains: chains(isDistressTolerance).length,
        missed_practice_chains: chains((chain) => chain.missedPractice).length,
        motivation_dip_chains: chains(isMotivationDip).length,
      },
      relapse_window_signal: state.relapseWindow,
      prediction_summary: {
        total: state.predictions.length,
        resolved_count: predictionStats.resolvedCount,
        occurred_count: predictionStats.occurredCount,
        partial_count: predictionStats.partialCount,
        did_not_occur_count: predictionStats.didNotOccurCount,
        occurred_ratio: predictionStats.occurredRatio,
        average_severity_gap: predictionStats.avgGap,
      },
      automatic_focus: {
        title: focus.title || null,
        focus: focus.focus || null,
        suggested_flow: focus.flow || null,
        status: "app_generated_hypothesis_not_clinical_conclusion",
      },
      // 삶의 경험 확장/만족도(expansion_score)와 같은 daily_checkin 카드에서 나오는 감사 기록.
      // 의도적으로 개선/악화 라벨을 붙이지 않으며, 건수·직전 대비 차이는 사실 수치로만 제공합니다.
      gratitude_reflection: {
        provenance: "AM",
        evidence_id: "AM-GRATITUDE-001",
        others_count: gratitudeMeasures.othersCount,
        self_count: gratitudeMeasures.selfCount,
        days_with_any_gratitude: gratitudeMeasures.daysWithAnyGratitude,
        checkin_days: gratitudeMeasures.checkinDays,
        compared_to_previous_snapshot: previousSnapshot
          ? {
              previous_others_count: previousSnapshot.gratitudeOthersCount ?? null,
              previous_self_count: previousSnapshot.gratitudeSelfCount ?? null,
              others_count_difference: Number.isFinite(previousSnapshot.gratitudeOthersCount) ? gratitudeMeasures.othersCount - previousSnapshot.gratitudeOthersCount : null,
              self_count_difference: Number.isFinite(previousSnapshot.gratitudeSelfCount) ? gratitudeMeasures.selfCount - previousSnapshot.gratitudeSelfCount : null,
            }
          : null,
        representative_entries: {
          others: pickRepresentativeGratitude(gratitudeMeasures.othersEntries).map(({ date, text, evidenceRef }) => ({ date, text, evidence_id: evidenceRef })),
          self: pickRepresentativeGratitude(gratitudeMeasures.selfEntries).map(({ date, text, evidenceRef }) => ({ date, text, evidence_id: evidenceRef })),
          selection_rule: "최근 기록 순 선정. 내용에 대한 긍정/부정 평가는 선정 기준에 포함되지 않음.",
        },
        limitation: "감사 항목은 선택 입력입니다. 건수·존재 여부만으로 반성의 진정성, 정서 개선 또는 치료 성실도를 판단할 수 없습니다.",
      },
      // 위험 연쇄/정서 주의/인지 주의 등 여러 지표가 이 임계값을 기준으로 계산됩니다.
      // 개인화가 적용된 경우, 어떤 방식으로 계산됐는지와 일반 기준값을 함께 남겨서
      // 다른 내담자의 문서와 판정 기준이 다를 수 있음을 문서 자체에서 알 수 있게 합니다.
      threshold_profile: {
        provenance: "AM",
        personalized: Boolean(state.thresholdProfile),
        active_thresholds: { ...state.thresholds },
        generic_defaults: { emotion: 7, cognitiveUrge: 6, urge: 8, action: 4, practice: 0 },
        calibration: state.thresholdProfile
          ? {
              method: state.thresholdProfile.method,
              applied_at: state.thresholdProfile.appliedAt,
              confirmed_by: state.thresholdProfile.confirmedBy,
              basis: state.thresholdProfile.basis,
              stale: isThresholdProfileStale(state.thresholdProfile),
            }
          : null,
        limitation: "문제행동수준(action) 임계값은 안전망으로 개인화하지 않습니다. 정서·인지·충동 임계값이 개인화된 경우, 이 내담자만의 기준이므로 다른 내담자 문서나 일반 기준과 수치를 직접 비교할 수 없습니다.",
      },
    },
    comparison: {
      previous_snapshot_available: Boolean(previousSnapshot),
      previous_snapshot: previousSnapshot,
      current_snapshot: currentSnapshot,
      metric_differences: buildSnapshotDifferences(previousSnapshot, currentSnapshot),
      limitation: "수치 변화만으로 치료효과, 진정성 또는 재범위험 변화를 결론내릴 수 없습니다.",
    },
    practice_summary: practiceGroups.map((group, index) => ({
      evidence_id: `AM-PRACTICE-${String(index + 1).padStart(3, "0")}`,
      provenance: "AM",
      name: group.name,
      planned_total: group.plannedTotal,
      completed_count: group.completedCount,
      completion_rate: group.completionRate,
      pleasure_average: group.pleasureAvg,
      mastery_average: group.masteryAvg,
      sample: group.sample || null,
    })),
    derived_chains: state.chains.map((chain, index) => ({
      evidence_id: `AM-CHAIN-${String(index + 1).padStart(3, "0")}`,
      provenance: "AM",
      source_chain_id: chain.chainId,
      date: formatDate(chain.date),
      scores: chain.scores,
      high_urge: chain.highUrge,
      actionized: chain.actionized,
      missed_practice: chain.missedPractice,
      summary: chainSummary(chain),
      status: "app_derived_pattern_requires_counselor_review",
    })),
    daily_reflection_summaries: state.reflectionDays.map((day, index) => ({
      evidence_id: `AM-REFLECTION-${String(index + 1).padStart(3, "0")}`,
      provenance: "AM",
      date: day.date || null,
      summary: day.text,
    })),
    source_evidence: sourceEvidence,
    counselor_evidence: counselorEvidence,
    clinical_continuity: aiContinuity,
    limitations: [
      "일상 행동의 상당 부분은 내담자의 자기보고이며 독립적으로 확인된 사실이 아닙니다.",
      "기록의 존재, 빈도 또는 문장 내용만으로 반성의 진정성을 판단할 수 없습니다.",
      "무기록일은 미실천과 동일하지 않고, 기록일은 실제 행동 시점과 다를 수 있습니다.",
      "앱 계산과 자동 연쇄는 상담자의 임상적 검토가 필요한 보조 정보입니다.",
      "감사 기록(gratitude_others/gratitude_self)은 선택 입력 항목입니다. 건수나 존재 여부만으로 반성의 진정성, 정서 개선 또는 치료 성실도를 판단할 수 없습니다.",
      "위험 판정에 쓰이는 정서·인지·충동 임계값이 개인화되어 있을 수 있습니다(threshold_profile 참고). 개인화된 경우 이 내담자만의 기준이므로 다른 내담자나 일반 기준과 수치를 직접 비교할 수 없습니다.",
      "관계적 영향(relational_impact)은 상담자가 선택적으로 입력하는 관찰이며, 내담자 자기보고나 독립적 확인을 대체하지 않습니다. 제3자(배우자·가족 등)의 신원 정보는 포함되어서는 안 됩니다.",
      "4번(개입 분기 판단) 단계는 특정 record_type에 대응하지 않는 순수 임상판단입니다. intervention_branch_rationale이 비어 있으면 그 판단 근거를 원자료만으로 재구성할 수 없습니다.",
      "safety_screening(자살·자해 위험 선별)은 표준화된 평가 도구가 아니라 상담자의 매 회기 선별 판단이며, 자살위험 진단을 대체하지 않습니다. 이전 회기 값이 자동으로 이어지지 않으므로 매 회기 새로 확인해야 하고, 어떤 외부 제출 문서에도 포함되지 않습니다.",
      "외부 제출 문서는 상담자가 원자료 대조, 비식별화, 사실 확인 및 최종 승인을 해야 합니다.",
    ],
  };
  packageData.validation = validateAiEvidencePackage(packageData);
  return packageData;
}

function validateAiEvidencePackage(packageData) {
  const errors = [];
  const warnings = [];
  const checks = [];
  const addCheck = (name, passed, severity, message) => {
    checks.push({ name, passed, severity, message });
    if (!passed) (severity === "error" ? errors : warnings).push(message);
  };

  addCheck("schema_version", packageData.schema_version === AI_EVIDENCE_SCHEMA, "error", `스키마가 ${AI_EVIDENCE_SCHEMA}가 아닙니다.`);
  addCheck("package_id", Boolean(packageData.package_metadata?.package_id), "error", "패키지 ID가 없습니다.");
  addCheck("eligible_rows", packageData.source_integrity?.eligible_row_count > 0, "error", "AI 근거자료에 사용할 수 있는 CSV 행이 없습니다.");
  addCheck(
    "source_count_match",
    packageData.source_evidence.length === packageData.source_integrity.eligible_row_count,
    "error",
    "사용 가능 행 수와 source_evidence 건수가 일치하지 않습니다.",
  );
  addCheck(
    "payload_parse",
    packageData.source_integrity.payload_parse_failure_evidence_ids.length === 0,
    "error",
    `payload_json 파싱 실패가 ${packageData.source_integrity.payload_parse_failure_evidence_ids.length}건 있습니다.`,
  );

  const evidenceItems = collectPackageEvidenceItems(packageData);
  const missingIds = evidenceItems.filter((item) => !item?.evidence_id).length;
  const evidenceIds = evidenceItems.map((item) => item.evidence_id).filter(Boolean);
  const duplicateIds = evidenceIds.filter((id, index) => evidenceIds.indexOf(id) !== index).filter((id, index, list) => list.indexOf(id) === index);
  addCheck("evidence_ids_present", missingIds === 0, "error", `근거 ID가 없는 항목이 ${missingIds}건 있습니다.`);
  addCheck("evidence_ids_unique", duplicateIds.length === 0, "error", `중복 근거 ID가 있습니다: ${duplicateIds.join(", ")}`);

  const invalidProvenance = evidenceItems.filter((item) => item?.provenance && !["SR", "AM", "CO", "AI", "CH"].includes(item.provenance));
  addCheck("provenance_codes", invalidProvenance.length === 0, "error", `허용되지 않은 출처 코드가 ${invalidProvenance.length}건 있습니다.`);
  addCheck("subject_alias", Boolean(packageData.package_metadata?.subject_alias) && !String(packageData.package_metadata.subject_alias).includes("UNKNOWN"), "warning", "내담자 비식별 코드가 없거나 임시 UNKNOWN 코드입니다.");
  addCheck("record_dates", Boolean(packageData.period_and_completeness?.first_record_date), "warning", "날짜가 있는 일상 기록을 확인할 수 없습니다.");
  addCheck("session_date", Boolean(packageData.treatment_participation?.session_date), "warning", "상담 회기일이 입력되지 않았습니다.");
  addCheck("attendance", packageData.treatment_participation?.attendance_status !== "not_recorded", "warning", "회기 참여 상태가 기록되지 않았습니다.");
  addCheck(
    "counselor_verified_fact",
    packageData.counselor_evidence.some((item) => item.evidence_type === "directly_verified_fact"),
    "warning",
    "상담자가 직접 확인한 사실이 입력되지 않았습니다.",
  );
  addCheck(
    "missing_days_reviewed",
    packageData.period_and_completeness.dates_without_daily_record_inside_observed_span.length === 0,
    "warning",
    `관찰기간 내부 무기록일이 ${packageData.period_and_completeness.dates_without_daily_record_inside_observed_span.length}일 있습니다.`,
  );
  addCheck(
    "delayed_entries_reviewed",
    packageData.period_and_completeness.delayed_entries_two_or_more_days.length === 0,
    "warning",
    `2일 이상 지연 작성된 기록이 ${packageData.period_and_completeness.delayed_entries_two_or_more_days.length}건 있습니다.`,
  );
  addCheck(
    "privacy_scan",
    packageData.privacy.potential_identifier_findings.length === 0,
    "warning",
    `개인정보 형식 후보가 ${packageData.privacy.potential_identifier_findings.length}곳에서 탐지되었습니다.`,
  );
  if (packageData.source_integrity.excluded_row_count > 0) {
    warnings.push(`스키마 또는 공유 모드 조건 때문에 ${packageData.source_integrity.excluded_row_count}행이 제외되었습니다.`);
  }

  return {
    validator_version: "maeumgoyo_ai_evidence_validator_v1",
...
function collectPackageEvidenceItems(packageData) {
  return [
    ...(packageData.source_evidence || []),
    ...(packageData.counselor_evidence || []),
    ...(packageData.clinical_continuity || []),
    ...(packageData.practice_summary || []),
    ...(packageData.derived_chains || []),
    ...(packageData.daily_reflection_summaries || []),
    packageData.current_period_app_measures,
  ].filter(Boolean);
}

function renderPreflightResult(container, validation, label, options = {}) {
  if (!container) return;
  const statusLabel = validation.status === "valid" ? "통과" : validation.status === "valid_with_warnings" ? "경고 있음" : "오류";
  const messages = [
    ...(validation.errors || []).map((message) => `<li><strong>오류:</strong> ${escapeHtml(message)}</li>`),
    ...(validation.warnings || []).map((message) => `<li><strong>경고:</strong> ${escapeHtml(message)}</li>`),
  ];
  const fixTarget = getFirstFixablePreflightTarget(validation);
  const fixButton = fixTarget
    ? `<button class="preflight-fix-button secondary small" type="button" data-fix-target="${escapeHtml(fixTarget.id)}">${escapeHtml(fixTarget.label)}로 이동</button>`
    : "";
  container.className = `preflight-result ${validation.status || "invalid"}`;
  container.innerHTML = `<strong>${escapeHtml(label)} 점검: ${statusLabel}</strong><span>오류 ${(validation.errors || []).length}건 · 경고 ${(validation.warnings || []).length}건</span>${fixButton}${messages.length ? `<ul>${messages.join("")}</ul>` : "<p>현재 자동점검에서 발견된 문제가 없습니다.</p>"}`;
  const button = container.querySelector(".preflight-fix-button");
  if (button) {
    button.addEventListener("click", () => confirmAndFocusPreflightTarget(button.dataset.fixTarget, button.textContent || "수정 위치"));
  }
  const toastType = validation.status === "valid" ? "success" : validation.status === "valid_with_warnings" ? "warning" : "error";
  showActionMessage(`${label} 점검 완료: ${statusLabel} · 오류 ${(validation.errors || []).length}건 · 경고 ${(validation.warnings || []).length}건`, toastType);
  if (options.askToMove && fixTarget) {
    window.setTimeout(() => {
      const shouldMove = window.confirm(`${label} 점검에서 수정할 수 있는 항목이 있습니다.\n\n"${fixTarget.label}" 위치로 이동하여 바로 수정할까요?`);
      if (shouldMove) focusPreflightTarget(fixTarget.id);
    }, 120);
  }
}

const PREFLIGHT_FIX_TARGETS = {
  csv_required: { id: "dropZone", label: "CSV 파일 불러오기" },
  session_date: { id: "sessionDateInput", label: "상담 회기일" },
  attendance: { id: "sessionAttendanceInput", label: "회기 참여 상태" },
  counselor_verified_fact: { id: "counselorVerifiedInput", label: "직접 확인한 사실" },
  verified_fact: { id: "counselorVerifiedInput", label: "직접 확인한 사실" },
  assigned_task: { id: "assignedTasksInput", label: "이번 주 상담과제" },
  verification_limit: { id: "verificationLimitsInput", label: "확인 범위와 한계" },
  clinical_continuity: { id: "acceptedAiSuggestions", label: "이번 회기 판단 기록" },
};

function getFirstFixablePreflightTarget(validation) {
  const failedChecks = (validation.checks || []).filter((check) => !check.passed);
  const errorTarget = failedChecks.find((check) => check.severity === "error" && PREFLIGHT_FIX_TARGETS[check.name]);
  const warningTarget = failedChecks.find((check) => PREFLIGHT_FIX_TARGETS[check.name]);
  const target = PREFLIGHT_FIX_TARGETS[(errorTarget || warningTarget || {}).name];
  return target || null;
}

function focusPreflightTarget(targetId) {
  ...
  window.setTimeout(() => {
    ...
    window.setTimeout(() => target.classList.remove("preflight-focus-target"), 2400);
  }, 260);
}

function confirmAndFocusPreflightTarget(targetId, label = "수정 위치") {
  const cleanLabel = String(label).replace(/로 이동$/, "");
  const shouldMove = window.confirm(`${cleanLabel} 위치로 이동하여 수정할까요?`);
  if (shouldMove) focusPreflightTarget(targetId);
}

function invalidatePreflightResults() {
  if (els.evidencePreflightResult) {
    els.evidencePreflightResult.className = "preflight-result";
    els.evidencePreflightResult.textContent = "자료가 변경되었습니다. 표준 근거자료를 다시 점검하세요.";
  }
  if (els.promptReadinessResult) {
    els.promptReadinessResult.className = "preflight-result";
    els.promptReadinessResult.textContent = "자료가 변경되었습니다. 선택 문서의 준비상태를 다시 점검하세요.";
  }
}

function buildSourceEvidenceItem(row) {
  const payload = parsePayload(row.payload_json);
  const prefix = "SR";
  const updatedTs = Date.parse(row.updated_at || "");
  const recordTs = row.date ? Date.parse(`${row.date}T00:00:00`) : NaN;
  const daysAfter = Number.isFinite(updatedTs) && Number.isFinite(recordTs)
    ? Math.max(0, Math.floor((updatedTs - recordTs) / (24 * 60 * 60 * 1000)))
    : null;
  return {
    evidence_id: `${prefix}-${sanitizeEvidenceId(row.id)}`,
    provenance: prefix,
    source_type: "client_app_entry",
    verification_status: "client_self_report_not_independently_verified",
    ...
    entry_timing: {
      days_after_record_date: daysAfter,
      is_delayed_two_or_more_days: daysAfter !== null && daysAfter >= 2,
    },
  };
}

const RELATIONAL_DISCLOSURE_LABELS = {
  not_recorded: "기록하지 않음",
  not_applicable: "해당사항 없음",
  undisclosed: "비공개(내담자만 인지)",
  partial_disclosure: "부분 공개",
  disclosed: "상대에게 공개함",
  discovered: "발각됨(비계획적 노출)",
};
const RELATIONAL_REPAIR_LABELS = {
  not_recorded: "기록하지 않음",
  not_applicable: "해당사항 없음",
  not_started: "회복 시도 전",
  in_progress: "회복 진행 중",
...
function buildCounselorEvidenceItems() {
  const items = [];
  const push = (id, type, text) => {
    const value = String(text || "").trim();
    if (!value) return;
    items.push({ evidence_id: id, provenance: "CO", evidence_type: type, verification_status: "counselor_entered", text: value });
  };
  push("CO-NOTE-001", "session_note", els.notes?.value);
  push("CO-TASK-001", "assigned_task", els.assignedTasksInput?.value);
  push("CO-VERIFY-001", "directly_verified_fact", els.counselorVerifiedInput?.value);
  push("CO-LIMIT-001", "verification_limit", els.verificationLimitsInput?.value);
  // 4번(개입 분기 판단)은 9단계 중 유일하게 특정 record_type에 대응하지 않는 순수 임상판단이라,
  // 그 근거를 남기지 않으면 나중에 "왜 이 개입을 택했는가"를 재구성할 수 없습니다.
  push("CO-BRANCH-001", "intervention_branch_rationale", els.interventionBranchNote?.value);
  push("CO-RELATIONAL-NOTE-001", "relational_impact_note", els.relationalImpactNote?.value);
  push("CO-SAFETY-001", "safety_screening_note", els.safetyScreeningNote?.value);
  return items;
}

function buildContinuityEvidenceItems() {
  const items = [];
  const push = (id, provenance, status, text) => {
    const value = String(text || "").trim();
    if (value) items.push({ evidence_id: id, provenance, status, text: value });
  };
  push("CO-CONTEXT-001", "CO", "imported_context_requires_current_counselor_review", state.previousAiContextText);
  push("CH-001", "CH", "counselor_adopted", els.acceptedAiSuggestions?.value);
  push("AI-DEFERRED-001", "AI", "deferred_not_adopted", els.deferredAiSuggestions?.value);
  push("CO-QUESTION-001", "CO", "planned_for_next_session", els.nextSessionQuestions?.value);
  push("CO-NEXT-001", "CO", "context_for_next_ai_use", els.nextAiContext?.value);
  return items;
}

function computeEvidenceDateCoverage(rows) {
  const dailyTypes = new Set(["observation", "practice_log", "prediction", "daily_checkin"]);
  const recordedDays = unique(rows.filter((row) => dailyTypes.has(row.record_type)).map((row) => row.date).filter(Boolean)).sort();
  if (!recordedDays.length) return { first: null, last: null, spanDays: 0, recordedDays: [], missingDays: [] };
  const first = recordedDays[0];
  const last = recordedDays[recordedDays.length - 1];
  const firstTs = Date.parse(`${first}T00:00:00`);
  const lastTs = Date.parse(`${last}T00:00:00`);
  const spanDays = Math.round((lastTs - firstTs) / (24 * 60 * 60 * 1000)) + 1;
  const recorded = new Set(recordedDays);
  const missingDays = [];
  for (let ts = firstTs; ts <= lastTs; ts += 24 * 60 * 60 * 1000) {
    const date = formatDate(new Date(ts));
    if (!recorded.has(date)) missingDays.push(date);
  }
  return { first, last, spanDays, recordedDays, missingDays };
}

function buildSnapshotDifferences(previous, current) {
  if (!previous) return [];
  return SNAPSHOT_METRICS.map(([key, label, unit, higherIsBetter]) => {
    const before = previous[key];
    const after = current[key];
    const difference = Number.isFinite(before) && Number.isFinite(after) ? after - before : null;
    return { key, label, unit, higher_is_better: higherIsBetter, previous: before ?? null, current: after ?? null, difference };
  });
}
...
function scanPotentialPersonalIdentifiers(value, evidenceId, path, findings) {
  if (value === null || value === undefined) return;
  if (Array.isArray(value)) {
    value.forEach((item, index) => scanPotentialPersonalIdentifiers(item, evidenceId, `${path}[${index}]`, findings));
    return;
  }
  if (typeof value === "object") {
    Object.entries(value).forEach(([key, item]) => scanPotentialPersonalIdentifiers(item, evidenceId, `${path}.${key}`, findings));
    return;
  }
  if (typeof value !== "string" || !value.trim()) return;
  const patterns = [
    ["email", /[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/i],
    ["phone_number", /(?:01[016789]|0\d{1,2})[-.\s]?\d{3,4}[-.\s]?\d{4}/],
    ["resident_registration_number", /\b\d{6}[-\s]?[1-4]\d{6}\b/],
    ["web_address", /https?:\/\/\S+/i],
  ];
  patterns.forEach(([type, regex]) => {
    if (regex.test(value)) findings.push({ evidence_id: evidenceId, field_path: path, candidate_type: type, value_not_included_in_warning: true });
  });
}

function exportSummary() {
  if (!state.records.length) {
    showSaveProblem("상담기록 요약을 저장할 자료가 없습니다. 먼저 CSV를 불러오거나 예시 자료를 실행하세요.");
    return;
  }
  if (!window.confirm("이번 회기 비교용 요약 파일에는 핵심 수치, 연쇄 요약, 상담 메모가 포함될 수 있습니다. 다음 회기 비교를 위해 승인된 보관 위치에 저장하시겠습니까?")) return;
  const predictionStats = predictionAccuracyStats(state.predictions);
  const practiceGroups = groupPractice();
  const gratitude = computeGratitudeMeasures();
  const prevGratitude = state.previousSnapshot
    ? { othersCount: state.previousSnapshot.gratitudeOthersCount ?? null, selfCount: state.previousSnapshot.gratitudeSelfCount ?? null }
    : null;
  const gratitudeSummary = {
    othersCount: gratitude.othersCount,
    selfCount: gratitude.selfCount,
    daysWithAnyGratitude: gratitude.daysWithAnyGratitude,
    checkinDays: gratitude.checkinDays,
    comparedToPreviousSummary: prevGratitude
      ? {
          previousOthersCount: prevGratitude.othersCount,
          previousSelfCount: prevGratitude.selfCount,
          othersCountDifference: Number.isFinite(prevGratitude.othersCount) ? gratitude.othersCount - prevGratitude.othersCount : null,
          selfCountDifference: Number.isFinite(prevGratitude.selfCount) ? gratitude.selfCount - prevGratitude.selfCount : null,
        }
      : null,
    interpretationLimit: "감사 기록은 선택 입력 항목입니다. 건수 증감만으로 정서 상태의 개선·악화나 치료 성실도를 판단하지 않습니다.",
  };
  const gratitudeHighlights = {
    others: pickRepresentativeGratitude(gratitude.othersEntries).map(({ date, text }) => ({ date, text })),
    self: pickRepresentativeGratitude(gratitude.selfEntries).map(({ date, text }) => ({ date, text })),
    note: "최근 기록 순으로 선정한 원문입니다. 선정 기준에 내용에 대한 평가는 포함되어 있지 않습니다.",
  };
  const payload = {
    app: "마음고요 상담분석실",
    documentTitle: "이번 회기 비교용 요약",
    documentPurpose: "현재 회기 기록 보관과 다음 회기 변화 비교를 위한 요약 파일입니다. AI 회기 프로토콜 생성을 직접 목적으로 하는 파일이 아닙니다.",
    intendedUse: [
      "이번 회기 상담기록 보관",
      "다음 회기에서 지난 회기 비교용 요약 JSON으로 불러와 변화 비교",
    ...
    thresholdProfile: state.thresholdProfile,
    metrics: {
      records: state.records.length,
      chains: state.chains.length,
  ...
  const triggerButton = document.activeElement === els.summaryArchiveBtn ? els.summaryArchiveBtn : els.exportBtn;
  downloadText(filename, JSON.stringify(payload, null, 2), "application/json;charset=utf-8", triggerButton);
}
...
function exportSessionPrepPackage() {
  ...
  if (!window.confirm("AI 회기준비 자료에는 최근 1~2주 핵심 문장, 프로젝트 연속성 프롬프트, 이전 회기연결 메모가 포함됩니다. 외부 AI에 사용하기 전 내담자 식별정보를 제거하세요.")) return;
...
function readPreviousAiContextFile(event) {
  ...
  reader.onload = () => {
    ...
    showImportMessages([`이전 회기연결 메모(${file.name})를 불러왔습니다.`]);
    renderMeta();
  };
  reader.readAsText(file);
}

async function exportAiSessionContext() {
  if (!state.records.length) {
    showImportMessages(["저장할 회기연결 메모가 없습니다. CSV를 먼저 불러오세요."]);
    return;
  }
  if (!window.confirm("AI 전달용 회기연결 메모는 다음 회기 AI 준비자료에 이어 붙일 치료 연속성 기록입니다. 내담자 식별정보가 들어가지 않았는지 확인했습니까?")) return;
...
function buildAiSessionContextText() {
  ...
  return [
    ...
    "## 1. 파일의 역할",
    "",
    "이 파일은 AI의 자동 메모리가 아니라, 상담자가 AI에게 다음 회기에도 이어서 참고시키고 싶은 치료 맥락 기록입니다.",
    "다음 회기의 `AI 회기준비 자료`를 만들 때 이전 회기연결 메모로 불러와 사용하세요.",
    "",
    "## 2. 프로젝트 정보",
    "",
    `- 비식별 자료 코드: ${clientAlias}`,
    `- AI 전달자료 이름표: ${projectName}`,
    `- 회기연결 메모 저장일: ${new Date().toISOString()}`,
    `- 자료 범위: ${state.rangeLabel || (state.rangeDays ? `최근 ${state.rangeDays}일` : "-")}`,
    ...
    "## 11. 4번(개입 분기) 판단과 근거",
    "",
    textOrDash(els.interventionBranchNote?.value),
    "",
    "## 12. 관계적 영향 (선택)",
    "",
    `- 공개·발각 상태: ${RELATIONAL_DISCLOSURE_LABELS[els.relationalDisclosureStatus?.value] || "기록하지 않음"}`,
    `- 관계 회복 진행 상태: ${RELATIONAL_REPAIR_LABELS[els.relationalRepairStatus?.value] || "기록하지 않음"}`,
    textOrDash(els.relationalImpactNote?.value),
    "",
    "## 13. 개인정보 보호 메모",
    "",
    "- 이 파일은 AI 전달자료 이름표로 관리하세요. 실제 이름을 사용할 경우 보안 폴더와 접근권한을 먼저 확인하세요.",
    "- 실제 이름, 연락처, 기관명, 특정 가능한 장소명은 외부 AI에 입력하기 전 제거하세요. 관계적 영향 메모에는 배우자·가족 등 제3자의 신원을 짐작할 수 있는 정보를 특히 더 주의하세요.",
  ].join("\n");
}
...
function buildSessionPrepText() {
  ...
  return [
    ...
    "## 2. 프로젝트 연속성 프롬프트",
    "",
    `이 내담자는 AI 전달자료 이름표 \`${projectName}\` 아래에서 회기별 분석과 제안을 축적한다.`,
    ...
    state.previousAiContextText
      ? `아래 내용은 상담자가 불러온 이전 AI 전달용 회기연결 메모(${state.previousAiContextFileName || "이전 메모"})입니다. 이번 회기 분석에서 반드시 참고하세요.\n\n${state.previousAiContextText}`
      : "불러온 이전 AI 전달용 회기연결 메모가 없습니다. 이번 회기 자료만 기준으로 분석하되, 다음 회기부터 축적할 항목을 제안하세요.",
    "",
    "## 4. AI에게 요청할 작업",
    "",
...
function hasUnsavedContent() {
  return state.records.length > 0 || [els.notes, els.assignedTasksInput, els.counselorVerifiedInput, els.verificationLimitsInput]
...
function resetAll(skipConfirm = false) {
  ...
  if (els.sessionAttendanceInput) els.sessionAttendanceInput.value = "not_recorded";
  if (els.safetyScreeningStatus) els.safetyScreeningStatus.value = "not_recorded";
  if (els.safetyScreeningNote) els.safetyScreeningNote.value = "";
  [els.assignedTasksInput, els.counselorVerifiedInput, els.verificationLimitsInput].forEach((input) => {
...
function resetDataOnly() {
  ...
  if (els.previousAiContextStatus) els.previousAiContextStatus.textContent = "이전 회기연결 메모를 아직 불러오지 않았습니다.";
...
function actionLabel(value) {
  ...
  return score === null ? "" : `문제행동수준 ${score}/5`;
}
...
function downloadText(filename, text, mimeType = "application/json;charset=utf-8", triggerButton = null) {
  const now = Date.now();
  const signature = `${filename}|${mimeType}|${String(text).length}`;
  if (state.lastSaveSignature === signature && now - state.lastSaveRequestedAt < 2000) {
    showActionToast("같은 저장 요청이 이미 처리 중이거나 방금 완료되었습니다.", "warning");
    return;
  }
  if (state.saveInProgress) {
    showActionToast("이미 저장이 처리 중입니다. 잠시 후 다시 시도하세요.", "warning");
    return;
  }
  state.lastSaveSignature = signature;
  state.lastSaveRequestedAt = now;
  state.saveInProgress = true;
  if (triggerButton) triggerButton.disabled = true;
  try {
    downloadTextViaBrowser(filename, text, mimeType, triggerButton);
  } finally {
    window.setTimeout(() => {
...
function downloadTextViaBrowser(filename, text, mimeType = "application/json;charset=utf-8", triggerButton = null) {
...
function showSaveComplete(filenameOrMessage, triggerButton = null) {
  const message = String(filenameOrMessage || "저장 요청 완료");
  const looksLikeFile = /\.(json|csv|txt|md)$/i.test(message);
  const finalMessage = looksLikeFile
    ? `저장 요청 완료: ${message} - 브라우저 다운로드 폴더 또는 선택한 저장 위치를 확인하세요.`
    : message;
  if (els.storageFolderStatus) els.storageFolderStatus.textContent = finalMessage;
  showActionToast(finalMessage, "success");
  flashSaveButton(triggerButton, "저장 요청 완료");
}

function flashSaveButton(triggerButton, label = "저장 요청 완료") {
  ...
  window.setTimeout(() => {
    ...
    triggerButton.classList.remove("saved");
  }, 2200);
}
...
function getStorageLabel() {
  const typed = state.clientStorageName || (els.clientStorageNameInput ? els.clientStorageNameInput.value.trim() : "");
  return typed || getClientAlias() || "client";
}

function getAiContextFileLabel() {
  const projectName = getProjectName();
  return projectName || getStorageLabel();
}
...
function buildSummaryFileName() {
  return `지난_회기_비교용_${sanitizeForFilename(getStorageLabel())}_${computeDataDateRangeLabel()}.json`;
}

function buildAiEvidenceFileName() {
  return `표준_근거자료_${sanitizeForFilename(getStorageLabel())}_${computeDataDateRangeLabel()}_ai-evidence-v1.json`;
}

function buildPromptProfileFileName(profile) {
  return `${sanitizeForFilename(getStorageLabel())}_prompt-${profile.code}_${sanitizeForFilename(profile.title)}_${APP_VERSION}.md`;
}

function buildSessionPrepFileName() {
  return `${sanitizeForFilename(getStorageLabel())}_${computeDataDateRangeLabel()}_ai-session-prep.md`;
}

function buildAiContextFileName() {
  return `${sanitizeForFilename(getAiContextFileLabel())}_${computeDataDateRangeLabel()}_AI전달용_회기연결메모.md`;
}

function buildAiReportFileName() {
  return `${sanitizeForFilename(getStorageLabel())}_${computeDataDateRangeLabel()}_ai-detailed-source.txt`;
}
