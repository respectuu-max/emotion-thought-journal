...
```json
{
  ...
  "gratitude_others": "친구가 먼저 연락해줘서 고마웠다",
...
- `gratitude_others`, `gratitude_self`: **v100부터 추가.** 감사 기록을 "주변 사람에 대한 감사"와 "나 자신에 대한 감사" 두 축으로 나눠 기록합니다(둘 다 선택 입력). 원래 관찰(observation) 레코드에 있던 `gratitude` 필드는 v93에서 입력 화면이 사라지며 항상 빈 문자열로만 남게 되었는데, 이 두 필드가 그 역할을 대신합니다. 관찰 기록은 문제 상황이 생길 때마다 여러 번 남길 수 있어 그 순간에 감사를 묻기엔 타이밍이 맞지 않는다고 판단해, 하루를 마무리하며 회고하는 이 record_type으로 옮겼습니다.
  - **이전 버전 CSV와의 호환**: v100 이전에 내보낸 CSV에는 이 두 필드가 없습니다. 이 경우 값이 없는 것으로(빈 문자열) 처리하면 되며, 오류로 취급하지 않습니다. 반대로 v100 이후 이 앱이 내보내는 CSV에는 이 필드가 항상 존재합니다(값이 비어 있어도 빈 문자열로 포함됩니다).
- **하루 1건 원칙**: 같은 날짜에 대해 여러 건이 존재하지 않습니다. 내담자가 같은 날 다시 저장하면 기존 레코드의 `id`가 유지된 채 `expansion_score`/`note`/`gratitude_others`/`gratitude_self`/`updated_at`만 갱신됩니다. 따라서 상담자 분석 앱은 `date`별로 정확히 0건 또는 1건만 있다고 가정해도 됩니다.
